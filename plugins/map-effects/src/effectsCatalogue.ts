 ...
 * `StylingModule.describe()` reads this catalogue unchanged.
 * @ignore
 */
 ...
 * solves. Ordering is the compositing order: bloom → grade → fog → edge blur → tint → vignette.
 * @ignore
 */
 ...
 * @group Map Effects
 */
export type EffectKnobValueOf<ID extends EffectKnobId> = (typeof effectKnobDefinitions)[ID]['kind'] extends 'color'
    ? string
    : number;
