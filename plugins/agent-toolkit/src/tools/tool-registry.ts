...
const defaultTools = {
    ...
    discoverPlaces: {
        ...
        classificationPrompt: 'Search for multiple places by text query or POI category within an area.',
        ...
        examples: [
            'discoverPlaces({ query: "coffee", where: { position: [4.9, 52.4] } })',
            'discoverPlaces({ poiCategories: ["ITALIAN_RESTAURANT"] })  // category-only, no query needed',
            'discoverPlaces({ query: "hotel", where: { boundingBox: [4.8, 52.3, 5.0, 52.5] } })',
            'discoverPlaces({ query: "parking", where: "Amsterdam" })  // geocoded area',
            'discoverPlaces({ query: "coffee", withinRange: "ranges-0" })',
            'discoverPlaces({ poiCategories: ["CAFE"], show: { markerType: "pin", zoomMode: "auto" } })',
        ],
        examplePrompts: [
    ...
    showPlaces: {
        ...
        classificationPrompt: 'Display a places entry on the map, replacing anything currently shown.',
        inputSchema: showPlacesSchema,
        outputSchema: showPlacesOutputSchema,
        execute: executeShowPlaces,
        tags: ['place', 'location'],
        examples: ['showPlaces()', 'showPlaces({ id: "places-2" })', 'showPlaces({ fitBounds: false })'],
        examplePrompts: ['Show the results on the map', 'Pin the search results', 'Display these places'],
        relatedTools: ['discoverPlaces', 'locatePlace', 'reverseGeocode', 'managePlaces', 'clearMap', 'flyTo'],
        dependsOn: ['discoverPlaces', 'locatePlace'],
    },
    managePlaces: {
        description: managePlacesDescription,
        classificationPrompt:
            'Add more places on top of whatever is currently shown, or hide specific place entries from the map.',
        inputSchema: managePlacesSchema,
        outputSchema: managePlacesOutputSchema,
        execute: executeManagePlaces,
        tags: ['place', 'location', 'map style'],
        examples: [
            'managePlaces({ add: ["places-2"] })',
            'managePlaces({ remove: ["places-1"] })',
            'managePlaces({ removeMatching: ["cinema"] })  // hide every entry labelled "cinema"',
            'managePlaces({ addMatching: ["cafe"] })',
            'managePlaces({ clear: true })',
            'managePlaces({ add: ["places-2"], fitBounds: true })',
        ],
        examplePrompts: [
        ...
        relatedTools: ['discoverPlaces', 'showPlaces', 'recallPlaces', 'clearMap'],
        dependsOn: ['discoverPlaces', 'locatePlace'],
    },
    showRoute: {
