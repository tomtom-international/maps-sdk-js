 ...
 *
 * Compact schema doc for the geometry polygons consumed/produced by sandboxed
 * code in `processData` / `analyseData` — injected per source as
 * `geometriesByEntry[key]`. Each item is a
...
export const GEOMETRIES_SCHEMA_DOC =
    'Each `geometriesByEntry[key]` (key = `${kind}:${id}`) is an array of Polygon/MultiPolygon Features. Each has `id` (the geometry-data-source id, ' +
