...
describe.skipIf(!MODEL)('setMapStandardStyle scenarios', { timeout: 180_000, retry: 3 }, () => {
    ...
    // SCENARIOS_FULL.
    const [canonical, ...rest] = getExamplePrompts('setMapStandardStyle');
    it(`classifies the canonical prompt: ${canonical}`, async () => {
        const outcome = await runToolScenario({ expectedTool: 'setMapStandardStyle', prompt: canonical });
    ...
    it.skipIf(!FULL_SCENARIOS).each(rest)('handles registry examplePrompt: %s', async (prompt) => {
        const outcome = await runToolScenario({ expectedTool: 'setMapStandardStyle', prompt });
        expect(outcome.success, outcome.failureReason).toBe(true);
    });
});
