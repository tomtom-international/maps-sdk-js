...
describe.skipIf(!MODEL)('zoomInOrOut scenarios', { timeout: 180_000, retry: 3 }, () => {
    ...
    // SCENARIOS_FULL.
    const [canonical, ...rest] = getExamplePrompts('zoomInOrOut');
    it(`classifies the canonical prompt: ${canonical}`, async () => {
        const outcome = await runToolScenario({ expectedTool: 'zoomInOrOut', prompt: canonical });
    ...
    it.skipIf(!FULL_SCENARIOS).each(rest)('handles registry examplePrompt: %s', async (prompt) => {
        const outcome = await runToolScenario({ expectedTool: 'zoomInOrOut', prompt });
        expect(outcome.success, outcome.failureReason).toBe(true);
    });
});
