 ...
 *   ...defaultTools.discoverPlaces,
 *   execute: async (args) => {
 *     console.log('[Search] Starting:', args);
 *     const startTime = Date.now();
 *
 *     const result = await defaultTools.discoverPlaces.execute(args);
 *
 ...
 * const agent = createMapAgent(map, {
 *   model: openai('gpt-4o'),
 *   tools: { discoverPlaces: loggingSearchTool }
 * });
 * ```
 *
 ...
 * import { createMapToolSet, createToolState } from '@tomtom-org/maps-sdk-plugin-ai-agent';
 *
 ...
 */
export function createMapToolSet(state: ToolState): DefaultToolSet {
...
export { TOOL_TAGS, type ToolTag } from './tool-tags';
