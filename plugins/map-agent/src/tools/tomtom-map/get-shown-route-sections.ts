...
/**
 * Create the get shown route sections tool.
 */
export function createGetShownRouteSectionsTool(state: ToolState): Tool {
    return tool({
        ...
        execute: async ({ sectionType }): Promise<z.infer<typeof getShownRouteSectionsOutputSchema>> => {
            try {
                ...
                const routes = shown.mainLines.features
                    .map((route) => {
                        const sectionArray = (route.properties.sections as SectionsProps)[
                            sectionType as keyof SectionsProps
                        ];
