...
import { extractMetrics, formatDate, metricsSchema, toDateString } from './get-traffic-area-analytics';
...
export const queryTrafficAnalyticsDescription =
    'Query cached traffic analytics (from getTrafficAreaAnalytics) without re-fetching. ' +
    'Filter by granularity, date range, hour range, day of week, or metric. ' +
    'Returns filtered entries plus the current visualization state (metric, mode displayed on map). ' +
    'Use for follow-up questions: breakdowns, patterns, or checking what is currently shown.';
