...
import type { LngLatLike, LayerSpecification, Map as MapLibreMap, MapGeoJSONFeature } from 'maplibre-gl';
...
export const waitForMapIdle = async (page: Page): Promise<void> => {
    await page.evaluate(async () => {
        const map = (globalThis as { mapLibreMap?: MapLibreMap }).mapLibreMap;
        if (!map) {
...
export const getLayerById = async (page: Page, layerId: string): Promise<LayerSpecWithSource> =>
    page.evaluate(
        (pageLayerId) =>
            (globalThis as { mapLibreMap?: MapLibreMap }).mapLibreMap
                ?.getStyle()
...
export const isLayerVisible = async (page: Page, layerId: string): Promise<boolean> =>
    page.evaluate(
        (inputLayerId) =>
            (globalThis as { mapLibreMap?: MapLibreMap }).mapLibreMap?.getLayoutProperty(inputLayerId, 'visibility') !==
            'none',
        layerId,
    );
...
export const getPaintProperty = async (page: Page, layerId: string, propertyName: string): Promise<unknown> =>
    page.evaluate(
        ({ layerID, propertyName: prop }) =>
            (globalThis as { mapLibreMap?: MapLibreMap }).mapLibreMap?.getPaintProperty(layerID, prop),
