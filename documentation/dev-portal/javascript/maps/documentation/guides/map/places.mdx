...
const places = await search({ query: "pharmacy", limit: 35, boundingBox: map.getBBox()});
const placesModule = await PlacesModule.init(map);
...
When you listen to Places events, the SDK will automatically apply [`eventState`](/javascript/maps/documentation/api-reference/modules/map_shared#SupportsEvents)
