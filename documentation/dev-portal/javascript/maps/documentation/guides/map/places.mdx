...
```javascript
import { GeoJSONPlaces } from "@anw/maps-sdk-js/map";
```
* Initialize a Places module (default config):
```javascript
const placesModule = await GeoJSONPlaces.init(map);
```
...
using the [`applyIconConfig()`](/javascript/maps/documentation/api-reference/classes/map_places.GeoJSONPlaces#applyConfig) method.
