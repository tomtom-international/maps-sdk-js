...
The [`GeoJSONPlaces`](/maps-sdk-js/javascript/maps/documentation/api-reference/classes/map.GeoJSONPlaces) module is used to show
...
const placesModule = await GeoJSONPlaces.init(sdkMap);

const places = await search({
    query: "pharmacy",
    limit: 35,
    boundingBox: sdkMap.getBounds()
});
...
It's possible to customize the Places marker icons in the [`GeoJSONPlaces`](/maps-sdk-js/javascript/maps/documentation/api-reference/classes/map.GeoJSONPlaces) module
using the `applyIconConfig()` method. See the [Map Places Customize](http://hosted-examples.maps-sdk-js.tomtom.com/main/map-places-customize) example.
