...
| Map styling (knobs)    | `map-styling.md`     | StylingModule, styling knobs, describe, catalogue, label size, icon size, road width, exit numbers, route shields, road arrows, 3D buildings, building footprints, POI micro markers, POI min zoom, zoom shift, POI label colour, traffic congestion colours, incident colours, restyle base map, brand the map, setMapStyling, describeMapStyling |
