# Changelog

All notable changes to this project will be documented in this file. See [standard-version](https://github.com/conventional-changelog/standard-version) for commit guidelines.

## [0.22.0](https://github.com/tomtom-international/maps-sdk-js/compare/v0.21.3...v0.22.0) (2025-10-21)


### Features

* Accept request params types to HasLngLat ([ab937ea](https://github.com/tomtom-international/maps-sdk-js/commit/ab937ea01843eaca24aa7e8357ca88c1f70a3a86))
* add geometry tag ([9e1d0b9](https://github.com/tomtom-international/maps-sdk-js/commit/9e1d0b9324ff23e01f4530ed9100a384a1d0dc0f))
* Add Matrix Routing support ([72a579d](https://github.com/tomtom-international/maps-sdk-js/commit/72a579d80f8adfeacc8d89b48d6b37b20af65bac))
* add missing maplibre css imports and change colors in tune with tomtom brand ([92787b1](https://github.com/tomtom-international/maps-sdk-js/commit/92787b18103e563fac023f7fa7f0ca693f945998))
* add new basic examples ([50add1f](https://github.com/tomtom-international/maps-sdk-js/commit/50add1ffc29fb5121bafc0ae2cccd03abd47c20a))
* add script to copy examples dist ([69102ee](https://github.com/tomtom-international/maps-sdk-js/commit/69102eedf7b237804a09cd1efc18a765e608655a))
* ai powered tsdoc improvements ([452c5d6](https://github.com/tomtom-international/maps-sdk-js/commit/452c5d6c776b0ff8715f6b8d894149ad29814b79))
* api reference integration in repo and npm ([c631771](https://github.com/tomtom-international/maps-sdk-js/commit/c631771abfde4934fb27462e7d955824d60dc009))
* api reference integration in repo and npm ([f69547c](https://github.com/tomtom-international/maps-sdk-js/commit/f69547c8c87f7bd6056359f9cafaa59bccf65ff6))
* api reference integration in repo and npm ([485ae07](https://github.com/tomtom-international/maps-sdk-js/commit/485ae07428bc0d3d7e9f59f0fb245a1899ec9980))
* api reference integration in repo and npm ([032a766](https://github.com/tomtom-international/maps-sdk-js/commit/032a766a2b42de6ea6316b6c5dd8a970d67d421d))
* api reference integration in repo and npm ([a1d88f7](https://github.com/tomtom-international/maps-sdk-js/commit/a1d88f72360668e7246c8af0e3cd61ac4f998b1a))
* api reference integration in repo and npm ([7631db9](https://github.com/tomtom-international/maps-sdk-js/commit/7631db9f5e920fbbccc0522c9c520750d04c4bd6))
* api reference integration in repo and npm ([9b753f9](https://github.com/tomtom-international/maps-sdk-js/commit/9b753f9653b31d0006b2cdef34a31d06932b54fb))
* api reference landing page with a link to the full api reference pages ([af0118f](https://github.com/tomtom-international/maps-sdk-js/commit/af0118f0bf7e302ea35259a9f2e793c989008a4c))
* **api reference:** add version labels to generated API reference ([e104fd5](https://github.com/tomtom-international/maps-sdk-js/commit/e104fd533f2b656fc47e6f2a41ef925cfc66876b))
* basic ldevr example ([0e5c951](https://github.com/tomtom-international/maps-sdk-js/commit/0e5c9510bec9e121c0678ab0f393bd039f2ea668))
* bring rest of relevant examples into monorepo ([a0c271e](https://github.com/tomtom-international/maps-sdk-js/commit/a0c271e364c6691f7bf79b2c8a96ce9e62d36b35))
* cleanup and improve example mdx content ([b2bf95d](https://github.com/tomtom-international/maps-sdk-js/commit/b2bf95d440f5e4484e7b01c3c3cd2376991f312f))
* do not link to source files from API reference since we consider them implementation details ([6841ae0](https://github.com/tomtom-international/maps-sdk-js/commit/6841ae05f32585851715261980b9b3f58b43853a))
* documentation directory improvements ([5d51f91](https://github.com/tomtom-international/maps-sdk-js/commit/5d51f91be41c4fdfc74c64c35ce512008691bf74))
* documentation improvements first iteration using ai agent ([5a2ac01](https://github.com/tomtom-international/maps-sdk-js/commit/5a2ac012793cd76eed06b4cc210d56ba8a327ff8))
* **EDXCE-118:** switching places services to Orbis ([0523751](https://github.com/tomtom-international/maps-sdk-js/commit/0523751aa0a127d09bc98ba017275fed686e2c99))
* **EDXCE-266:** migrating ev charging station by id to new orbis endpoint, with included refactoring ([fe49162](https://github.com/tomtom-international/maps-sdk-js/commit/fe49162b979072250dd983376c7632f7d4fae514))
* **EDXCE-275-EDXCE-282:** upgrading to maplibre 4.2.0 with disabled pending tiles canceling, adding waypoint event tests, and splitting events tests ([cbc798b](https://github.com/tomtom-international/maps-sdk-js/commit/cbc798b15f1d8e5677768709728708b8b5d1fd6f))
* **EDXCE-275-EDXCE-282:** upgrading to maplibre 4.2.0 with disabled pending tiles canceling, adding waypoint event tests, and splitting events tests ([9d16719](https://github.com/tomtom-international/maps-sdk-js/commit/9d16719a811c66f0f4777c06dd9b864e1ec54043))
* ensure coverage reports are picked up ([e0f0964](https://github.com/tomtom-international/maps-sdk-js/commit/e0f09642f43c957d836b705d83fde691103fa156))
* **ev:** new optional service parameter to filter out ev stations which have unknown availability ([cd2519b](https://github.com/tomtom-international/maps-sdk-js/commit/cd2519b8bc8a21225bd868babd81a5bbc535e3dc))
* **guides:** update guide versions on release ([4aadf3e](https://github.com/tomtom-international/maps-sdk-js/commit/4aadf3ec3cc6c441962735390d678adfcafcd7d9))
* **hillshade:** applying final hillshade styles ([9f65c50](https://github.com/tomtom-international/maps-sdk-js/commit/9f65c50b7e443e0d9de45d160beb137ed6d61647))
* improve css imports ([fec5815](https://github.com/tomtom-international/maps-sdk-js/commit/fec5815703de67eac57f3e3c9f88a2220b5fda66))
* improve how places are rendered ([7ad8205](https://github.com/tomtom-international/maps-sdk-js/commit/7ad8205b37d7b9a0a10a3b2b47f1787eefce33c4))
* include examples page in docs portal page as content ([88105ad](https://github.com/tomtom-international/maps-sdk-js/commit/88105ad7118e484fcecc12e7c201f67bdd13a9d9))
* **map:** adding more poi categories into map category groups ([a19e3a2](https://github.com/tomtom-international/maps-sdk-js/commit/a19e3a2ea86fb7bbab1d0d4fed048c6404abe992))
* **map:** allowing multiple base map modules with different layer groups ([8c7efeb](https://github.com/tomtom-international/maps-sdk-js/commit/8c7efebc2fe70f1af15c05a8558b0fe1933bb10f))
* **map:** allowing to change the style while keeping the previous style parts auto-adjusted to the new style variant ([3747973](https://github.com/tomtom-international/maps-sdk-js/commit/37479739f7413f2009306035d05853d3ffd54425))
* **map:** allowing to change visibility for selected layer groups in a base map module ([6bdb146](https://github.com/tomtom-international/maps-sdk-js/commit/6bdb146b285e757d12af4e5b5e8c222de866c96d))
* **map:** allowing to have multiple geometries modules for a map ([98fd8b1](https://github.com/tomtom-international/maps-sdk-js/commit/98fd8b1043cc9592630f44f19862e511c895183d))
* **map:** changing how we add parameters to setStyle to allow them to grow better in the future ([a546e0a](https://github.com/tomtom-international/maps-sdk-js/commit/a546e0ad64cab08875d33790be82d63f246d7a7d))
* **map:** default attribution control showing map copyrights ([d7f6248](https://github.com/tomtom-international/maps-sdk-js/commit/d7f6248a14e629eb833c2bb59cdf3eff02be1aaf))
* **map:** enhancing events to allow different handlers for the same source but different layers ([3510d3b](https://github.com/tomtom-international/maps-sdk-js/commit/3510d3b47afe62261c42e8334cef58831cdc4bf8))
* **map:** first ability to programmatically put and clean event states ([6cf2d26](https://github.com/tomtom-international/maps-sdk-js/commit/6cf2d264916cd270e2cdc6339cbce7b105655b61))
* **map:** improvements displaying charging stations along route ([f9f55ad](https://github.com/tomtom-international/maps-sdk-js/commit/f9f55ad0297c368aae74c80452ad0ef6fefd26c1))
* **map:** increasing event delays trying to make tests pass with default ci/cd capacity ([c0e6115](https://github.com/tomtom-international/maps-sdk-js/commit/c0e6115673b2d0d0ea39ddfe17121dff20fff8de))
* **map:** switching map display to Orbis ([ba45361](https://github.com/tomtom-international/maps-sdk-js/commit/ba45361ece1f193b9d7cde85834d1e4affad8e6f))
* move all html and css files from examples under src ([8989360](https://github.com/tomtom-international/maps-sdk-js/commit/8989360a08e59791131d1e9dc8f515a4b6f730aa))
* move constants from docs portal to examples ([a5d62eb](https://github.com/tomtom-international/maps-sdk-js/commit/a5d62eb3e1df77df021a01bf2a8deaefb2954939))
* **oauth2:** experimental support for oauth2 for api access ([85e8c67](https://github.com/tomtom-international/maps-sdk-js/commit/85e8c67c08306857972a1b4f92ede21bb8546b7a))
* pass info about response status to onAPIResponse ([72af83c](https://github.com/tomtom-international/maps-sdk-js/commit/72af83c1bbd886ee7a2fc6e7d10a5b7e89dbe6ea))
* pin improvements including ev charging stops ([a00dca8](https://github.com/tomtom-international/maps-sdk-js/commit/a00dca8bfaf77dab89e7d39ec8b5ffe309029a08))
* **places:** ability to conveniently show a single place or an array of them in addition to a feature collection ([874cd2a](https://github.com/tomtom-international/maps-sdk-js/commit/874cd2af40f907f01dcc24b2f6bcbcdf3f58c246))
* **places:** improving place by id response and opening hours properties, and dep upgrades ([4b4b0b2](https://github.com/tomtom-international/maps-sdk-js/commit/4b4b0b281cf60a5bde8c589a3b0c68fa24f7bb48))
* **places:** typing poicategory in place responses. ([34c114f](https://github.com/tomtom-international/maps-sdk-js/commit/34c114fdfcc468a35d52e8ebed1922a674c90a56))
* **pois:** removing unused init param from pois module ([5d929eb](https://github.com/tomtom-international/maps-sdk-js/commit/5d929eb568fc537af95f28bcc5044517464fc945))
* remove api reference from navigation file ([b0ab06d](https://github.com/tomtom-international/maps-sdk-js/commit/b0ab06d36c5facdde2dd7ce1f2e6ff476b7ceaa9))
* remove async wrappers from examples by using esnext on tsconfig ([8d07b9d](https://github.com/tomtom-international/maps-sdk-js/commit/8d07b9d5cb42c611a72644740ccd8f628ba6c8f6))
* restoring and improving routing vehicle params ([9a4e3ae](https://github.com/tomtom-international/maps-sdk-js/commit/9a4e3aebcc6d4c52ae9a954bb51139b5f7bfa8b4))
* restoring and improving routing vehicle params ([443b95d](https://github.com/tomtom-international/maps-sdk-js/commit/443b95df483dc6660e70ed1591cd231dd50ea4ec))
* restoring pins on the map based on new assets ([7cd992c](https://github.com/tomtom-international/maps-sdk-js/commit/7cd992cbe259f42bc5d376d42b20bb468d8d92c4))
* restructure tags filter ([c96387f](https://github.com/tomtom-international/maps-sdk-js/commit/c96387f8d145c6881c5aef720d6d5f0e916859d1))
* **routing:** adding avoid options and sections ([79d1570](https://github.com/tomtom-international/maps-sdk-js/commit/79d15707fb08c4d6a0d5122d9a5518da89d6e7aa))
* **routing:** adding basic entry points support when calculating routes and displaying waypoints ([fdc4f48](https://github.com/tomtom-international/maps-sdk-js/commit/fdc4f486cad45b65695b02c70d8ff1a513e53fb2))
* **routing:** adding path point index for each instruction when parsing a route ([303480c](https://github.com/tomtom-international/maps-sdk-js/commit/303480c4856e7bf4e171e6433611668ce16d3a5a))
* **routing:** allowing for default 0 value in maxAlternatives for convenience ([f36088f](https://github.com/tomtom-international/maps-sdk-js/commit/f36088fcf46b915dedb1fdd2ae10e5aef5df1e92))
* **routing:** automatically updating summary bubbles if distance units config is changed ([1dee6cf](https://github.com/tomtom-international/maps-sdk-js/commit/1dee6cf36688747a673bf0a307a5b9b98ed81114))
* **routing:** cleaning up ldevr post parameters from sdk which should be better integrated in a future, to prevent examples with overlapping or confusing inputs. Updating map integration tests data file locations. ([9132307](https://github.com/tomtom-international/maps-sdk-js/commit/9132307b825396bf3b375a56b518e9d415c65281))
* **routing:** displaying guidance instructions along route line ([9a423c2](https://github.com/tomtom-international/maps-sdk-js/commit/9a423c2a1fa413e663b6915cb6ca90fc49cdee37))
* **routing:** first iteration for displaying bubbles with route summaries on the map ([21580bd](https://github.com/tomtom-international/maps-sdk-js/commit/21580bdc4d1ebba7ed62680ab386d568668d3d50))
* **routing:** improving routing module tests ([325b910](https://github.com/tomtom-international/maps-sdk-js/commit/325b9108b17f191e3ffb996d973029e885e37516))
* **routing:** improving types ([c7a5b56](https://github.com/tomtom-international/maps-sdk-js/commit/c7a5b568ae09f9de59de136199769cb0ffbfe048))
* **routing:** making some guidance and api version parameters optional, adding ldver and guidance routing module tests ([ada7c01](https://github.com/tomtom-international/maps-sdk-js/commit/ada7c01559f72ba2edd17e4f18d139000030c03f))
* **routing:** migrating to orbis routing v2 ([b9bdeb7](https://github.com/tomtom-international/maps-sdk-js/commit/b9bdeb72d528c719f1922063df9c7817ae831a49))
* **routing:** updated local svg-based waypoint assets and route line colors ([566be95](https://github.com/tomtom-international/maps-sdk-js/commit/566be95b756f7bde211112cf9f12b98bdaf73e92))
* run blackduck workflow on PR and main pushes, populate result in a PR comment ([ba21b44](https://github.com/tomtom-international/maps-sdk-js/commit/ba21b44242674d0906050b890ae21765e4ff3c7b))
* **sdk:** removing poi style part since in orbis pois are now part of the base map. Pins are still not supported, when/if they come it'll require another sdk update ([bdbd320](https://github.com/tomtom-international/maps-sdk-js/commit/bdbd320bcea6e908d4b4e48ab0701a12489d5b5c))
* **services:** adding optional callbacks for api requests and responses within each service ([9c63526](https://github.com/tomtom-international/maps-sdk-js/commit/9c63526ab125cbd1f735f344ad30d81af783dc2c))
* **services:** calling onAPIRequest also when APIs respond with errors ([0db1943](https://github.com/tomtom-international/maps-sdk-js/commit/0db194394ca8a27f036df56a1dc0eb814a9dbd48))
* **services:** exporting fetch types to facilitate overrides and api monitoring by clients ([697898e](https://github.com/tomtom-international/maps-sdk-js/commit/697898ee1364f44d518238438a4236b9bedbecf1))
* **services:** reachable range service ([23dce6b](https://github.com/tomtom-international/maps-sdk-js/commit/23dce6b6f8b1f552db8a0caca95f060aa9368ec4))
* **services:** removing axios and replacing it with fetch, together with nodejs 21 support ([17cd7a9](https://github.com/tomtom-international/maps-sdk-js/commit/17cd7a95ec7c8fbfc932753aa0ed5da6278d3e1f))
* simplify example tags and groupings ([ec7a93c](https://github.com/tomtom-international/maps-sdk-js/commit/ec7a93c16814bbb19384f5726212ad17aff7b92d))
* switch from published to standard styles for clarity ([a163111](https://github.com/tomtom-international/maps-sdk-js/commit/a16311130457bb69dda3dd956d072cf9244a768e))
* switch to monorepo with examples ([dca8997](https://github.com/tomtom-international/maps-sdk-js/commit/dca8997c364a162f2f1ab157bdae6f68f2f3c754))
* typedoc improvements ([e59119a](https://github.com/tomtom-international/maps-sdk-js/commit/e59119ab469b12f9dc1ea9ac0fecdc70e918a93f))
* **units:** enabling new global and local configurations for distance and time display units ([212e78e](https://github.com/tomtom-international/maps-sdk-js/commit/212e78e98063a856d112ac28add21768acf4adc3))
* **units:** enabling new global and local configurations for distance and time display units ([e2d27d8](https://github.com/tomtom-international/maps-sdk-js/commit/e2d27d873906a1d6efe14ba4166ac26d3041a747))
* use release-please to bump examples version ([9f4afb5](https://github.com/tomtom-international/maps-sdk-js/commit/9f4afb5e1d39c766ad59a435ce48d4ce4c7d24ab))
* use version of the root package.json ([cb3b277](https://github.com/tomtom-international/maps-sdk-js/commit/cb3b277c979eeba320f60ffdce78d4af94788071))


### Bug Fixes

* add .netrc file needed for blackduck scan ([64e4323](https://github.com/tomtom-international/maps-sdk-js/commit/64e4323756abb7c71641b43cc917b81d766df9e6))
* add install step ([a19527f](https://github.com/tomtom-international/maps-sdk-js/commit/a19527f9008b49ac591d50cdf887c2e7bcc2689f))
* add missing typedoc ([243716f](https://github.com/tomtom-international/maps-sdk-js/commit/243716f96f0dd633326bbbfbd93a96a598b88255))
* added bdtrc file ([a0f5631](https://github.com/tomtom-international/maps-sdk-js/commit/a0f56317b732508195a705d11fea2a31bb3da573))
* api reference and docs navigation generation ([83a6149](https://github.com/tomtom-international/maps-sdk-js/commit/83a6149780495a344ed68984c2bb36bb1216ec95))
* api reference workflow ([c16f1ac](https://github.com/tomtom-international/maps-sdk-js/commit/c16f1ac6848f5d4b07e330367112590900e069a7))
* api reference workflow to react better to triggers ([915db75](https://github.com/tomtom-international/maps-sdk-js/commit/915db75faaf5528f79af689ac08b539f9b7c64c2))
* api reference workflow to react better to triggers ([f228680](https://github.com/tomtom-international/maps-sdk-js/commit/f2286800291dfb720067186a8a82208844546ebb))
* attempt to mitigate style changes vs geojson race conditions ([777068f](https://github.com/tomtom-international/maps-sdk-js/commit/777068fef2246aca0cdc2985fcbe8bc36329bbc2))
* blackduck runner ([3b735a6](https://github.com/tomtom-international/maps-sdk-js/commit/3b735a663bd1b167f334316ee1d368a439f656a4))
* branch cleanup action ([2d60484](https://github.com/tomtom-international/maps-sdk-js/commit/2d60484284041fa7920ed0c434585728ebca5425))
* change fail on severity to be only for the very high errors ([94debb0](https://github.com/tomtom-international/maps-sdk-js/commit/94debb06cfb96cc07ff85d50046141b5113d0aaa))
* **charging_stations:** adding title and adding features for alternatives ([cf305fd](https://github.com/tomtom-international/maps-sdk-js/commit/cf305fdc1d23b3636b61888b07df5870325b1d37))
* **charging_stations:** work in progress ([9bf4dd2](https://github.com/tomtom-international/maps-sdk-js/commit/9bf4dd2de0ee8146939f74021155d72d224601f6))
* **charging_stations:** work in progress ([0df4efe](https://github.com/tomtom-international/maps-sdk-js/commit/0df4efeba19e754c646a76350b213fa11076bd5b))
* commit all possible doc changes in release-please ([1815200](https://github.com/tomtom-international/maps-sdk-js/commit/1815200de75d3cc0c36689df32b1b79f5cfd49fb))
* compress thumbnail images in examples ([458dbb6](https://github.com/tomtom-international/maps-sdk-js/commit/458dbb673290fb518216991b232eaf25f5651cb4))
* **core:** improvin polygon feature typing ([3cabe2c](https://github.com/tomtom-international/maps-sdk-js/commit/3cabe2c534238b9b4d2d9df93b08ce70aac43f62))
* documentation structure ([8c68019](https://github.com/tomtom-international/maps-sdk-js/commit/8c68019c53dbd90b6a04fd4ca27dc44303a9f4bb))
* e2e test setup and misc improvements ([dc8dbed](https://github.com/tomtom-international/maps-sdk-js/commit/dc8dbed0049f6a6904074d0266d933fc6a32a247))
* **EDXCE-282:** ensuring display waypoints always have feature IDs, which is needed for events and always good to have ([2972bdf](https://github.com/tomtom-international/maps-sdk-js/commit/2972bdf29076873f12a783197b41ed9a1d82fbf0))
* **EDXCE-282:** ensuring display waypoints always have feature IDs, which is needed for events and always good to have ([8d2aed8](https://github.com/tomtom-international/maps-sdk-js/commit/8d2aed8533ac84d1ff368a5f9648f28f33dfe1c3))
* **edxce-337:** removing legacy tollRoad section type in favor of the new "toll" one, while keeping tollVignette even if not supported yet ([5618ea2](https://github.com/tomtom-international/maps-sdk-js/commit/5618ea234d9b26d2ab18a49bd4c6a00b8227c33f))
* **EDXCE-89:** Adding TODO comments and updating a test ([0e527e1](https://github.com/tomtom-international/maps-sdk-js/commit/0e527e159b5ccb5432441ee3c6115a51bd6910d9))
* **EDXCE-89:** improvements ([c46b7ec](https://github.com/tomtom-international/maps-sdk-js/commit/c46b7ecf60e4f0df873adc43c7017cc39e2322ee))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis improvements and some cleanup ([301e549](https://github.com/tomtom-international/maps-sdk-js/commit/301e5495e8b0d89e521b595b5dde941e393226ba))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis, work in progress, added ldEV ([86bb078](https://github.com/tomtom-international/maps-sdk-js/commit/86bb0786b75355f85835645e90a13bb966a15626))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis, work in progress, fixing tests ([d99ebe6](https://github.com/tomtom-international/maps-sdk-js/commit/d99ebe68b2d2d5296d7e66ea78698ce9751887ae))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis, work in progress, lots of commented out code ([bce943a](https://github.com/tomtom-international/maps-sdk-js/commit/bce943af607101483e70ed3305ebc521bf33c8c8))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis, work in progress, reverting back vehicleRestricted ([ff354de](https://github.com/tomtom-international/maps-sdk-js/commit/ff354de86b721f56615f3962e9bd23d07bc88529))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis, work in progress, support for new sections ([50a5720](https://github.com/tomtom-international/maps-sdk-js/commit/50a5720ecd91eb4b49d8ea96ee5b6b677e89de3e))
* emit cjs builds for examples ([2cb2093](https://github.com/tomtom-international/maps-sdk-js/commit/2cb2093ae7f8ccbb3d402c26f67fb222b75a3f63))
* ensure appname is full repo with owner ([3e8b0e9](https://github.com/tomtom-international/maps-sdk-js/commit/3e8b0e997cf0f04121ac30c692634ff12055d9cd))
* error handling logic ([a2bda56](https://github.com/tomtom-international/maps-sdk-js/commit/a2bda562b181e9c43276911b51f981ec8d020910))
* fetch test after changing returned response format ([068b61c](https://github.com/tomtom-international/maps-sdk-js/commit/068b61cc1a265f0e0cbe5c936e37fd4a2a60f231))
* formatting html and css with biome ([dc18883](https://github.com/tomtom-international/maps-sdk-js/commit/dc1888306b510715bab389dde15ca60dedbd8937))
* further reduce thumbnails in resolution and size ([4528809](https://github.com/tomtom-international/maps-sdk-js/commit/45288093a7261370b005173cb1ea96b75a3f9fb0))
* **geometries:** small typing simplification / fix ([cd185ab](https://github.com/tomtom-international/maps-sdk-js/commit/cd185abca2fd94b59f4fb815069a7e929343543f))
* imports ([dc60baf](https://github.com/tomtom-international/maps-sdk-js/commit/dc60bafd21baf1b79a1b743b56b601a66709509e))
* include only src folders for biome linting ([824a5d3](https://github.com/tomtom-international/maps-sdk-js/commit/824a5d3fe263c856751d3f5f67dc251a8b1fd979))
* keep sdk version in sync with docs ([aaf93d4](https://github.com/tomtom-international/maps-sdk-js/commit/aaf93d457a8788bf5c8bfcd8bebdfc61d56e8965))
* make all functions to be iife ([a3c2d4e](https://github.com/tomtom-international/maps-sdk-js/commit/a3c2d4e42a6f94e53845b78c02494c53027c868e))
* map features after breaking style changes ([7e1cda7](https://github.com/tomtom-international/maps-sdk-js/commit/7e1cda759add15621fb1fbd846444c24b6deca5c))
* **map:** fixing petrol station map poi mapping ([a3592c3](https://github.com/tomtom-international/maps-sdk-js/commit/a3592c3d32d654dd4ff74b8dfccb7fe4f9ebb37a))
* **map:** hardening logic when styles and module changes are called quickly in succession ([4d271ce](https://github.com/tomtom-international/maps-sdk-js/commit/4d271ceda0a1937e5ea7aa3ed4e03fb43392ab44))
* **map:** hardening logic when styles and module changes are called quickly in succession ([2fa4a65](https://github.com/tomtom-international/maps-sdk-js/commit/2fa4a652bfd5b24967d0af2d03d08671fc5ab52e))
* **map:** hardening logic when styles and module changes are called quickly in succession ([fabc15d](https://github.com/tomtom-international/maps-sdk-js/commit/fabc15d27a9128123071e97091c09843d4e2a447))
* **map:** hotfix to allow supplying valid 0s as inputs ([b6496b6](https://github.com/tomtom-international/maps-sdk-js/commit/b6496b6ac9e560866522dd7554fe51dc69374452))
* **map:** moving summary bubble layer to the top ([0e446f3](https://github.com/tomtom-international/maps-sdk-js/commit/0e446f3df25d9498a8d62b2d7bbdf4e1c08fe91c))
* **map:** multiple fixes related to changing style vs events and showing geojson items ([2e9470c](https://github.com/tomtom-international/maps-sdk-js/commit/2e9470c331087aa4f77d00ef72ad40cc3180d9cc))
* **map:** removing maxzoom for charging stations on route for now ([af21c80](https://github.com/tomtom-international/maps-sdk-js/commit/af21c80eda250c26aeb678bd0f364f17b2900cc1))
* **map:** trying to allow for style data handler errors and still call the following ones. And minor misc fixes. ([f11d459](https://github.com/tomtom-international/maps-sdk-js/commit/f11d459c47745757da21c0733fca549a47466b98))
* **misc:** fixing tests and adding EV accessibility enum ([c4f81a0](https://github.com/tomtom-international/maps-sdk-js/commit/c4f81a0be363284b3a3609c5687913493bb8e84d))
* **misc:** removing incremental tsconfig setting to try to fix rollup bundling ([21239cd](https://github.com/tomtom-international/maps-sdk-js/commit/21239cd336edd8117d18450b2d86d802925972b2))
* missing maplibre css import ([a11a8ee](https://github.com/tomtom-international/maps-sdk-js/commit/a11a8eecc3117da7d1530cb8bd6064f152ca1dbf))
* missing typedoc ([98b8b53](https://github.com/tomtom-international/maps-sdk-js/commit/98b8b53f9b458bd64cfae1278930fa312b492c38))
* only update and commit docs to release branch before a release is created ([4f8f237](https://github.com/tomtom-international/maps-sdk-js/commit/4f8f2375b9b68a9c2acb9beca6977c790cad946a))
* pin location ([cdcc2a3](https://github.com/tomtom-international/maps-sdk-js/commit/cdcc2a31d5af710474defd5615a82732fab25a7f))
* **places:** attempting to mitigate strange glitch where showFeatures isn't updated in sdk geojson sources ([65c701b](https://github.com/tomtom-international/maps-sdk-js/commit/65c701b93f67a148d9b8b5ccf421528173a0a057))
* quick reductions of example titles ([d0bfc43](https://github.com/tomtom-international/maps-sdk-js/commit/d0bfc43933aa2a48e214bfb5badf9d36320808af))
* release action ([7e32f66](https://github.com/tomtom-international/maps-sdk-js/commit/7e32f669637e7c8ca8e77d3b5421ca768c1a7b44))
* release action ([4f85aa3](https://github.com/tomtom-international/maps-sdk-js/commit/4f85aa32ef281af62ce4b234ae11e59b18039fb7))
* remove redundant page title ([63f6c8e](https://github.com/tomtom-international/maps-sdk-js/commit/63f6c8e97a195509fdbbf844f3d101bbc17fd29e))
* removing noisy line at the end of each example, once we resume e2e testing we'll look into it ([08e1683](https://github.com/tomtom-international/maps-sdk-js/commit/08e1683ce1cf13d981841394bc6bca800430aa12))
* rename project in veracode, blackduck scan workflows ([cbee9ff](https://github.com/tomtom-international/maps-sdk-js/commit/cbee9ff3e6950e8cad08c6c9a34425f5756fbad4))
* resolve lint errors ([1945b9f](https://github.com/tomtom-international/maps-sdk-js/commit/1945b9fd11bb466d675090e130518ea67cc7faa1))
* revert script accidental change ([9c07c2b](https://github.com/tomtom-international/maps-sdk-js/commit/9c07c2b40d5f4ba4a9e13b43ef30f534a00e8744))
* **revgeo:** temporarily skipping flaky test, to be re-added later in a safer way ([f2b86ac](https://github.com/tomtom-international/maps-sdk-js/commit/f2b86ac629511c85dc165d905c084b69700fa9ea))
* **routing:** attempting to have the routing layers ready as soon as possible and then schedule loading images ([948a60d](https://github.com/tomtom-international/maps-sdk-js/commit/948a60d30d9ef71363bf98a583c1741a1750f7d1))
* **routing:** correcting section type inputs ([6eac931](https://github.com/tomtom-international/maps-sdk-js/commit/6eac931939f0db219a6b5559a844799d2c24e45b))
* **routing:** fixing parsing of toll roads, hardening section parsing and improving position utils ([d89369d](https://github.com/tomtom-international/maps-sdk-js/commit/d89369d3a2d3ea637a39ca5c389981b565ef8d94))
* **routing:** fixing the mapping of instructions to path points ([ee1227c](https://github.com/tomtom-international/maps-sdk-js/commit/ee1227cbf6768172bb983d52e81844c7f7579d04))
* **routing:** fixing waypoints rendering tests ([d2d9d18](https://github.com/tomtom-international/maps-sdk-js/commit/d2d9d18aa8540ebfc4330cf64d039fae5f2c373b))
* **routing:** hardening the display of instruction lines on the map ([a525aaf](https://github.com/tomtom-international/maps-sdk-js/commit/a525aafa80c5c3373645623b7647cf3a4502cbff))
* **routing:** hotfix to make route summary bubbles display more compact and mitigate unwanted word wrapping ([87d9d6a](https://github.com/tomtom-international/maps-sdk-js/commit/87d9d6ab75911605b1d6285fae6d11b68d85cfc9))
* **routing:** hotfix to show ev charging stations above most other layers on routes ([0e2e53a](https://github.com/tomtom-international/maps-sdk-js/commit/0e2e53a708433e8b16e5cb5e45637104b276e33e))
* **routing:** test stability hotfix ([c5b1998](https://github.com/tomtom-international/maps-sdk-js/commit/c5b19989b57c2deb441b140bb8d6223051e89137))
* **routing:** trying to fix ts check in tests project ([e111d69](https://github.com/tomtom-international/maps-sdk-js/commit/e111d694b1ef2410fadf879a4ffb850b520ef6b2))
* schema test ([a0ff144](https://github.com/tomtom-international/maps-sdk-js/commit/a0ff1443c322f377b2c7572178fda99e14f832ef))
* schemas and tests after upgrading ([e518132](https://github.com/tomtom-international/maps-sdk-js/commit/e518132a3987b2c1e2372d12b4d20ccc4df8076b))
* **sdk:** flaky test mitigation ([b73f5a7](https://github.com/tomtom-international/maps-sdk-js/commit/b73f5a7c9fc741bfe383f23bbf4b4078baf8861f))
* **sdk:** helping tests run better in ci cd ([aa0d334](https://github.com/tomtom-international/maps-sdk-js/commit/aa0d334b52c390e74416439a12d5eef8826bd95a))
* **sdk:** trying to make the geojson modules more resilient to quick changes in succession ([83f576b](https://github.com/tomtom-international/maps-sdk-js/commit/83f576b83aa3885780e08feb502bc1714ea30b55))
* **sdk:** trying to stabilize tests in ci cd ([094e297](https://github.com/tomtom-international/maps-sdk-js/commit/094e29728ebbb7e5450a001bbf8454efa0629954))
* **server:** attempt to skip loading the document object when in non-browser environments ([b3cd17d](https://github.com/tomtom-international/maps-sdk-js/commit/b3cd17d1a4e731f290c0fdeca22644ca46e755da))
* **server:** attempt to skip loading the document object when in non-browser environments ([f4c6317](https://github.com/tomtom-international/maps-sdk-js/commit/f4c63179333d3595d58f20ef4f14ff158e2ce996))
* **server:** attempt to skip loading the document object when in non-browser environments ([ab561e8](https://github.com/tomtom-international/maps-sdk-js/commit/ab561e89b2cdcdc0afc926f7b333952e2f14c095))
* **server:** attempt to skip loading the document object when in non-browser environments ([43bd883](https://github.com/tomtom-international/maps-sdk-js/commit/43bd883141cb8f9b70b40da6fac85509f1544542))
* **services:** hot fixing unstable routing integration test ([9bd06cb](https://github.com/tomtom-international/maps-sdk-js/commit/9bd06cbfa4e99ae65d3bf530464f93e4b427db5a))
* supporting receiving undefined connectors for charging stations ([7586cb3](https://github.com/tomtom-international/maps-sdk-js/commit/7586cb37ab030fa5441697006af64b55308c33e8))
* **tests:** enabling map mocked tests with local asset mocking ([bc5db13](https://github.com/tomtom-international/maps-sdk-js/commit/bc5db130e4e1425d0541ff878cf249616e6b36e0))
* **tests:** enabling puppeteer tests to still run while SDK imports trigger detection of document object due to embedded sdk assets ([5d1356f](https://github.com/tomtom-international/maps-sdk-js/commit/5d1356f5cc1fe3a89b3b28077e5ad3a1db3c5ce3))
* **tests:** hardening POI map tests, removing unnecessary and misleading assertion, since changing POI filters doesn't mean number of visible POIs always changes ([11020a9](https://github.com/tomtom-international/maps-sdk-js/commit/11020a9e5e0c63dea4d5851769a2ecce649e7b05))
* **tests:** stabilizing traffic incident tests for situations where we only have road closures available ([bb41f6b](https://github.com/tomtom-international/maps-sdk-js/commit/bb41f6b08211220055a2e46ab3934f33695304ab))
* **tests:** updating search test IDs after another search api Orbis migration ([eb0b497](https://github.com/tomtom-international/maps-sdk-js/commit/eb0b497172dd1e50b214615543008c86fb05e6ed))
* **test:** trying to stabilize test ([35b9505](https://github.com/tomtom-international/maps-sdk-js/commit/35b9505e87b870ad9d6c9195462e61be2f88613f))
* **test:** trying to stabilize test ([df71ca9](https://github.com/tomtom-international/maps-sdk-js/commit/df71ca9c453396f7a8770b3f8ef4bf939a114ebc))
* **traffic:** removing confusing unused type ([fbd73cf](https://github.com/tomtom-international/maps-sdk-js/commit/fbd73cfc7a3b74967f13759a93c407c8a429d32e))
* undo unwanted vite config changes ([3626037](https://github.com/tomtom-international/maps-sdk-js/commit/362603741e168a186f1d8b74916db3541533a965))
* unused variables as warning to make basic examples code more lenient ([6e87586](https://github.com/tomtom-international/maps-sdk-js/commit/6e875866d5539d81d694eca64f484cd368de3e0c))
* update all developer portal links to docs portal ([2e3a3ef](https://github.com/tomtom-international/maps-sdk-js/commit/2e3a3eff64a7397e6e964c3adec6d9c0b6aa5df4))
* update blackduck runner ([2fef6b6](https://github.com/tomtom-international/maps-sdk-js/commit/2fef6b6b575d3c305e81239b0a1b674c83e9092f))
* update image to thumbnail property ([27e6bec](https://github.com/tomtom-international/maps-sdk-js/commit/27e6bece3226d69fe5a9196263522975d303080c))
* update prop in release please config ([9aad792](https://github.com/tomtom-international/maps-sdk-js/commit/9aad79208aa8d8bdf58d995158d8548893aa9a86))
* update tags for examples ([6b920a5](https://github.com/tomtom-international/maps-sdk-js/commit/6b920a55c586e46f6de460993b66687268297c3d))
* update thumbnail and show json in html ([d299369](https://github.com/tomtom-international/maps-sdk-js/commit/d299369c4460bf284222195625df570b79129b98))
* updated version of blackduck action ([93b63c8](https://github.com/tomtom-international/maps-sdk-js/commit/93b63c8d5bedc6ea1e109a15fa7651357932f6a4))
* upgrade blackduck and python versions ([8d0c9eb](https://github.com/tomtom-international/maps-sdk-js/commit/8d0c9eb709bd808f793b8a9fd57149154d49741c))
* use bash for script executions for compatibility ([9266f15](https://github.com/tomtom-international/maps-sdk-js/commit/9266f15e16585eabffb52fd8099f57cdc7ec22cb))
* use existing examples package json ([86e97c4](https://github.com/tomtom-international/maps-sdk-js/commit/86e97c43e5bd40f9d60a2fc6fe89215f9edcf056))
* workflow calling wrong tests ([1af5772](https://github.com/tomtom-international/maps-sdk-js/commit/1af577241aa13a88bb5588004e396ad4c3bb5e6a))

## [0.21.3](https://github.com/tomtom-international/maps-sdk-js/compare/v0.21.2...v0.21.3) (2025-10-17)


### Bug Fixes

* imports ([9ee7ee8](https://github.com/tomtom-international/maps-sdk-js/commit/9ee7ee834b98e58fa7ce2c41e3bde66566117464))

## [0.21.2](https://github.com/tomtom-international/maps-sdk-js/compare/v0.21.1...v0.21.2) (2025-10-17)


### Bug Fixes

* unused variables as warning to make basic examples code more lenient ([5c16885](https://github.com/tomtom-international/maps-sdk-js/commit/5c1688584d44ef3ca6ae718e10ed1a5de7c40cca))

## [0.21.1](https://github.com/tomtom-international/maps-sdk-js/compare/v0.21.0...v0.21.1) (2025-10-17)


### Bug Fixes

* removing noisy line at the end of each example, once we resume e2e testing we'll look into it ([edc7c00](https://github.com/tomtom-international/maps-sdk-js/commit/edc7c00246659a4375cb05d4214b460acc8bb217))

## [0.21.0](https://github.com/tomtom-international/maps-sdk-js/compare/v0.20.0...v0.21.0) (2025-10-17)


### Features

* **855:** splitting traffic incidents into its own module ([d2d569e](https://github.com/tomtom-international/maps-sdk-js/commit/d2d569eef18b63f439b566185d31ea2179890d30))
* Accept request params types to HasLngLat ([2cf10b7](https://github.com/tomtom-international/maps-sdk-js/commit/2cf10b7af852efaef86189110716a7a52f247c8f))
* add geometry tag ([8372313](https://github.com/tomtom-international/maps-sdk-js/commit/837231330a0d3d569aff26eb046681a4a2e45de4))
* Add Matrix Routing support ([6f65f21](https://github.com/tomtom-international/maps-sdk-js/commit/6f65f21c6f25aef45e94e5da2b541f5500d35bdb))
* add missing maplibre css imports and change colors in tune with tomtom brand ([a2869ff](https://github.com/tomtom-international/maps-sdk-js/commit/a2869ff728051f4c3349bd11ed238ee8aa8f30af))
* add new basic examples ([10bb460](https://github.com/tomtom-international/maps-sdk-js/commit/10bb460d7588dad2d6278c5aeefa93a12684de53))
* add script to copy examples dist ([ccd41cb](https://github.com/tomtom-international/maps-sdk-js/commit/ccd41cbc888d879ededbcca0778c5ba25987ce78))
* api reference integration in repo and npm ([d22f808](https://github.com/tomtom-international/maps-sdk-js/commit/d22f808992b9f123550de277d1e77f5d35c7322f))
* api reference integration in repo and npm ([2d7a257](https://github.com/tomtom-international/maps-sdk-js/commit/2d7a2577b61fdec74abc0f195227f2f0c47b48ad))
* api reference integration in repo and npm ([b55675a](https://github.com/tomtom-international/maps-sdk-js/commit/b55675a24741ef3688365b42a883cb2a233be92e))
* api reference integration in repo and npm ([0341bc1](https://github.com/tomtom-international/maps-sdk-js/commit/0341bc17052b588e73090eb845112ee86df4f720))
* api reference integration in repo and npm ([af31b93](https://github.com/tomtom-international/maps-sdk-js/commit/af31b93aba905166cbbf6bec90e62b01bcb8032d))
* api reference integration in repo and npm ([28c7815](https://github.com/tomtom-international/maps-sdk-js/commit/28c78151c63eadae5875f4c930ccfbe18373c54e))
* api reference integration in repo and npm ([67486bb](https://github.com/tomtom-international/maps-sdk-js/commit/67486bb5edcecc6bd423d0a0653aaa7885c5ae79))
* api reference landing page with a link to the full api reference pages ([6e3ecc1](https://github.com/tomtom-international/maps-sdk-js/commit/6e3ecc1198b796435c6167fa00e6df98d968bc27))
* **api reference:** add version labels to generated API reference ([fa8d3fd](https://github.com/tomtom-international/maps-sdk-js/commit/fa8d3fdba07dc0c6f41041b7dd1518b45b81ff45))
* basic ldevr example ([b463982](https://github.com/tomtom-international/maps-sdk-js/commit/b46398202b30da8cfb4075545cb5922612f439c9))
* bring rest of relevant examples into monorepo ([3acedb7](https://github.com/tomtom-international/maps-sdk-js/commit/3acedb711f1a1ea499e184922281d2c116b7255e))
* cleanup and improve example mdx content ([2a77eef](https://github.com/tomtom-international/maps-sdk-js/commit/2a77eefbd14c18556b59bc2628d095f2c0a26589))
* do not link to source files from API reference since we consider them implementation details ([1a69f60](https://github.com/tomtom-international/maps-sdk-js/commit/1a69f60c9810a529ecdbc5fd3bd900db53ebbf82))
* **docs:** updates in project setup guide based on feedback ([ff45f6b](https://github.com/tomtom-international/maps-sdk-js/commit/ff45f6bf700b0ac7dafc389a3ae39a5437589d38))
* documentation directory improvements ([4bb3b53](https://github.com/tomtom-international/maps-sdk-js/commit/4bb3b53aa6c2a3edeb7fc361bbb101c3542df540))
* documentation improvements first iteration using ai agent ([808ef0a](https://github.com/tomtom-international/maps-sdk-js/commit/808ef0aaf46e97327c8021e66fa2fbf049042cdc))
* **EDXCE-118:** switching places services to Orbis ([053cadb](https://github.com/tomtom-international/maps-sdk-js/commit/053cadbd037859a31a9c71f24f2e316d9833d28b))
* **EDXCE-266:** migrating ev charging station by id to new orbis endpoint, with included refactoring ([5b055e2](https://github.com/tomtom-international/maps-sdk-js/commit/5b055e238422c3c967987b09504f29dc17c54bc2))
* **EDXCE-275-EDXCE-282:** upgrading to maplibre 4.2.0 with disabled pending tiles canceling, adding waypoint event tests, and splitting events tests ([4d644bd](https://github.com/tomtom-international/maps-sdk-js/commit/4d644bdcfb3095708ab054949e29d45c5bc53746))
* **EDXCE-275-EDXCE-282:** upgrading to maplibre 4.2.0 with disabled pending tiles canceling, adding waypoint event tests, and splitting events tests ([de6ac02](https://github.com/tomtom-international/maps-sdk-js/commit/de6ac02ef4076439483d5a7fd8329b0bd21d552c))
* ensure coverage reports are picked up ([3d8f6ed](https://github.com/tomtom-international/maps-sdk-js/commit/3d8f6ed40130d469d32d06d940ea1f0bcd0d40df))
* **ev:** new optional service parameter to filter out ev stations which have unknown availability ([5971aa3](https://github.com/tomtom-international/maps-sdk-js/commit/5971aa3d1a7f771eebaca3a002ae7b295ef91830))
* **guides:** update guide versions on release ([5afc554](https://github.com/tomtom-international/maps-sdk-js/commit/5afc554540acf6476d66a7198004a913fcaa4e5d))
* **hillshade:** applying final hillshade styles ([d22e4b3](https://github.com/tomtom-international/maps-sdk-js/commit/d22e4b390f92711b1632b0a5592784589cab2165))
* improve css imports ([42d490a](https://github.com/tomtom-international/maps-sdk-js/commit/42d490a995f36611f84ad5f63d8e26b323ba8565))
* improve how places are rendered ([494498d](https://github.com/tomtom-international/maps-sdk-js/commit/494498d381c203f7c531ea8df173741fffe7dc99))
* include examples page in docs portal page as content ([dd85db2](https://github.com/tomtom-international/maps-sdk-js/commit/dd85db26ca3cb320405847cd3a1b3878e3e429f8))
* **map:** adding more poi categories into map category groups ([a8fce4e](https://github.com/tomtom-international/maps-sdk-js/commit/a8fce4eb778ce67572352fa3ea386c500abd7063))
* **map:** allowing multiple base map modules with different layer groups ([dd5d372](https://github.com/tomtom-international/maps-sdk-js/commit/dd5d3724b73c44db47cb6093cc66a92d1744aab3))
* **map:** allowing to change the style while keeping the previous style parts auto-adjusted to the new style variant ([78e6d3b](https://github.com/tomtom-international/maps-sdk-js/commit/78e6d3b5bd6f9e8e5160868b8630baf66bcb7b69))
* **map:** allowing to change visibility for selected layer groups in a base map module ([d40cac9](https://github.com/tomtom-international/maps-sdk-js/commit/d40cac9092348dce567e48965ac7a60eec668ca0))
* **map:** allowing to have multiple geometries modules for a map ([98a9caa](https://github.com/tomtom-international/maps-sdk-js/commit/98a9caa1e67c2a80cee4296b4f5ab41e30b3dd62))
* **map:** changing how we add parameters to setStyle to allow them to grow better in the future ([77e66fb](https://github.com/tomtom-international/maps-sdk-js/commit/77e66fb2beaa99453aa369933c684053dadde852))
* **map:** default attribution control showing map copyrights ([a1551bb](https://github.com/tomtom-international/maps-sdk-js/commit/a1551bb7b47137a6055441322ae272f870ed2b5b))
* **map:** enhancing events to allow different handlers for the same source but different layers ([be37a76](https://github.com/tomtom-international/maps-sdk-js/commit/be37a76fb863b11df902495d343d50aec3283321))
* **map:** first ability to programmatically put and clean event states ([927d2eb](https://github.com/tomtom-international/maps-sdk-js/commit/927d2eb4b392ab9357dafdaf1b385a36b3eb58b2))
* **map:** improvements displaying charging stations along route ([304894b](https://github.com/tomtom-international/maps-sdk-js/commit/304894b8c04fda866e65f63d0a0cf39fc03bba4b))
* **map:** increasing event delays trying to make tests pass with default ci/cd capacity ([3a58a28](https://github.com/tomtom-international/maps-sdk-js/commit/3a58a2812494a3564081e638956cb1de4c09dbd7))
* **map:** switching map display to Orbis ([cdee49f](https://github.com/tomtom-international/maps-sdk-js/commit/cdee49f875440e64ed007e4996c56fc2a9c7ab26))
* move all html and css files from examples under src ([ac76444](https://github.com/tomtom-international/maps-sdk-js/commit/ac76444b9469cc43176673786b817fad7a9274c8))
* move constants from docs portal to examples ([6068c85](https://github.com/tomtom-international/maps-sdk-js/commit/6068c85874447e54ecf8feac5546c7190c4c9b88))
* **oauth2:** experimental support for oauth2 for api access ([92ad4cc](https://github.com/tomtom-international/maps-sdk-js/commit/92ad4cc5f7a78f72f709cbdd0b35c81e4171160e))
* pass info about response status to onAPIResponse ([c84bc59](https://github.com/tomtom-international/maps-sdk-js/commit/c84bc59268b0ac0b69966c91679684b78c8a5a8e))
* pin improvements including ev charging stops ([f045079](https://github.com/tomtom-international/maps-sdk-js/commit/f0450798c015fc574f0783589fb9a8e895a0d159))
* **places:** ability to conveniently show a single place or an array of them in addition to a feature collection ([7f3d9b0](https://github.com/tomtom-international/maps-sdk-js/commit/7f3d9b0774de48efde5b72828e09232815d0c970))
* **places:** improving place by id response and opening hours properties, and dep upgrades ([d804a99](https://github.com/tomtom-international/maps-sdk-js/commit/d804a993e1d7e9cdc7fe0220d1e709715aee0561))
* **places:** typing poicategory in place responses. ([cc6253e](https://github.com/tomtom-international/maps-sdk-js/commit/cc6253e771eb6dcd27505ea4b4a550edb2005703))
* **pois:** removing unused init param from pois module ([37152ea](https://github.com/tomtom-international/maps-sdk-js/commit/37152ea3cf440c9f665babe805186ac9dc9a138b))
* remove api reference from navigation file ([cee8e1d](https://github.com/tomtom-international/maps-sdk-js/commit/cee8e1d1dfed2248967b915ca88a2363bdcc945c))
* remove async wrappers from examples by using esnext on tsconfig ([891f0cf](https://github.com/tomtom-international/maps-sdk-js/commit/891f0cf0e574644c083d3d985f1e93d3b730a34c))
* restoring and improving routing vehicle params ([77fb752](https://github.com/tomtom-international/maps-sdk-js/commit/77fb75267acb8b613794edac9b01722ca5d56f47))
* restoring and improving routing vehicle params ([992ee48](https://github.com/tomtom-international/maps-sdk-js/commit/992ee48e6518f78395b8292090b07ed5a8caec23))
* restoring pins on the map based on new assets ([18ea84a](https://github.com/tomtom-international/maps-sdk-js/commit/18ea84a652e579c366d59197a06f94b48e2e0057))
* restructure tags filter ([f3373f3](https://github.com/tomtom-international/maps-sdk-js/commit/f3373f38812fa2d395b845f0fb3a6f1396ce3391))
* **routing:** adding avoid options and sections ([8a4dd7b](https://github.com/tomtom-international/maps-sdk-js/commit/8a4dd7ba39a064ce2b02442d292ed1dd041bc1e4))
* **routing:** adding basic entry points support when calculating routes and displaying waypoints ([4310f28](https://github.com/tomtom-international/maps-sdk-js/commit/4310f28caac9ebda29a2cf5b6bbb4bdbec49a2d8))
* **routing:** adding path point index for each instruction when parsing a route ([b30a430](https://github.com/tomtom-international/maps-sdk-js/commit/b30a43049cc8160fb6befb6d7551b9ecc3c193dc))
* **routing:** allowing for default 0 value in maxAlternatives for convenience ([45a7a62](https://github.com/tomtom-international/maps-sdk-js/commit/45a7a62164040c56508a15566af6a91b260d60e6))
* **routing:** automatically updating summary bubbles if distance units config is changed ([17853d3](https://github.com/tomtom-international/maps-sdk-js/commit/17853d31e0c96c94a199659b38c8edc0cad3c08e))
* **routing:** cleaning up ldevr post parameters from sdk which should be better integrated in a future, to prevent examples with overlapping or confusing inputs. Updating map integration tests data file locations. ([ce1a2b2](https://github.com/tomtom-international/maps-sdk-js/commit/ce1a2b2ca0e84788c62317acbcabf77fa63234f5))
* **routing:** displaying guidance instructions along route line ([6a0c0c9](https://github.com/tomtom-international/maps-sdk-js/commit/6a0c0c9d7da931542a4930ffa307bd6a2a94210a))
* **routing:** first iteration for displaying bubbles with route summaries on the map ([f7ddbcd](https://github.com/tomtom-international/maps-sdk-js/commit/f7ddbcd267e431e7122efb2445e7f97bd9bdab6f))
* **routing:** improving routing module tests ([ef74ae5](https://github.com/tomtom-international/maps-sdk-js/commit/ef74ae53c6a9121d0201af04e4cf5eb00120e2eb))
* **routing:** improving types ([d92a9e9](https://github.com/tomtom-international/maps-sdk-js/commit/d92a9e99679aa143b3a21e25914da7464c382f5a))
* **routing:** making some guidance and api version parameters optional, adding ldver and guidance routing module tests ([7173863](https://github.com/tomtom-international/maps-sdk-js/commit/71738633feb99399adbcdb16653fe7f927c7a0d6))
* **routing:** migrating to orbis routing v2 ([94cb276](https://github.com/tomtom-international/maps-sdk-js/commit/94cb276699a942553f3e028ad7ba93a5989ceca2))
* **routing:** updated local svg-based waypoint assets and route line colors ([7b3e5cc](https://github.com/tomtom-international/maps-sdk-js/commit/7b3e5cc0a4f31117a38969d060189201530f89a7))
* run blackduck workflow on PR and main pushes, populate result in a PR comment ([509d7ba](https://github.com/tomtom-international/maps-sdk-js/commit/509d7ba5e0513a0b9a808bd7c5f13e35439936d9))
* **sdk:** removing poi style part since in orbis pois are now part of the base map. Pins are still not supported, when/if they come it'll require another sdk update ([33403f0](https://github.com/tomtom-international/maps-sdk-js/commit/33403f0e6904c44c3ef5bf8357d99df7854b261b))
* **services:** adding optional callbacks for api requests and responses within each service ([7e3f149](https://github.com/tomtom-international/maps-sdk-js/commit/7e3f1494534ca51ba57ef196ed134264c644abbf))
* **services:** calling onAPIRequest also when APIs respond with errors ([8521fb4](https://github.com/tomtom-international/maps-sdk-js/commit/8521fb4ba4ca069a483d23177db6d94f691146dc))
* **services:** exporting fetch types to facilitate overrides and api monitoring by clients ([973782c](https://github.com/tomtom-international/maps-sdk-js/commit/973782cf1bcd250bf3765c7ef30bfae1bcbd93ee))
* **services:** reachable range service ([c67b4ae](https://github.com/tomtom-international/maps-sdk-js/commit/c67b4ae57d1a4a1e31405e85b3ee8a5d0fea34eb))
* **services:** removing axios and replacing it with fetch, together with nodejs 21 support ([f136083](https://github.com/tomtom-international/maps-sdk-js/commit/f136083e1d5535dbe65997d3ef997c18e95dc7b8))
* simplify example tags and groupings ([68326a7](https://github.com/tomtom-international/maps-sdk-js/commit/68326a7a310d65834213d508460bca33d7e8b3f6))
* switch from published to standard styles for clarity ([c03aeeb](https://github.com/tomtom-international/maps-sdk-js/commit/c03aeeb7e1c7c14a52ec1ab54e7eba08fffc37be))
* switch to monorepo with examples ([34ddd02](https://github.com/tomtom-international/maps-sdk-js/commit/34ddd0221734652dd0db675dc4133c9b6811eb41))
* typedoc improvements ([10b68c2](https://github.com/tomtom-international/maps-sdk-js/commit/10b68c2c30f4c53e99ab3458395d3321d33160b7))
* **units:** enabling new global and local configurations for distance and time display units ([fa246d3](https://github.com/tomtom-international/maps-sdk-js/commit/fa246d3dafda8185e09e7f712aa35c3a3b73fd28))
* **units:** enabling new global and local configurations for distance and time display units ([7ff1bca](https://github.com/tomtom-international/maps-sdk-js/commit/7ff1bcab598d0492597e7176f82b0fdc13c7f456))
* use release-please to bump examples version ([d9804e5](https://github.com/tomtom-international/maps-sdk-js/commit/d9804e50f3efdf3383431b2faf6ab2c26df2530e))
* use version of the root package.json ([ce0ebf2](https://github.com/tomtom-international/maps-sdk-js/commit/ce0ebf26aac1323b524822e953e81df439883218))


### Bug Fixes

* add .netrc file needed for blackduck scan ([b9398b6](https://github.com/tomtom-international/maps-sdk-js/commit/b9398b6d80815e9628e8ea79764cecd71f502ef4))
* add install step ([00c044f](https://github.com/tomtom-international/maps-sdk-js/commit/00c044f214d0a1ac8993348564d9444f584579b6))
* add missing typedoc ([cbd2994](https://github.com/tomtom-international/maps-sdk-js/commit/cbd29948d3769054759d2177e7be404e945d1946))
* added bdtrc file ([de82a2c](https://github.com/tomtom-international/maps-sdk-js/commit/de82a2c0e49fcc569fb4ddc3fd2984ede75af68f))
* api reference and docs navigation generation ([13dd7ab](https://github.com/tomtom-international/maps-sdk-js/commit/13dd7abcf9681194e9cca3c188689f5a7ec73592))
* api reference workflow ([9911edd](https://github.com/tomtom-international/maps-sdk-js/commit/9911edd14b47138fbe0c292b39397603aa2cf061))
* api reference workflow to react better to triggers ([93ed1b9](https://github.com/tomtom-international/maps-sdk-js/commit/93ed1b94fb21ce75a85e320928b61d50c2f13d47))
* api reference workflow to react better to triggers ([130bdc5](https://github.com/tomtom-international/maps-sdk-js/commit/130bdc536c7b9aef808cc74fccea0f7dc401d0b3))
* attempt to mitigate style changes vs geojson race conditions ([2e61829](https://github.com/tomtom-international/maps-sdk-js/commit/2e61829a139b7783c62d6558338ade262216e00e))
* blackduck runner ([7980133](https://github.com/tomtom-international/maps-sdk-js/commit/798013365cd6e2c3249b1c8ea35986a5d0cfb464))
* branch cleanup action ([a589526](https://github.com/tomtom-international/maps-sdk-js/commit/a589526f4510519a9744ffff6257c5280c8a00ca))
* change fail on severity to be only for the very high errors ([77643a4](https://github.com/tomtom-international/maps-sdk-js/commit/77643a425f6a46b2ce1015bf04c7b785e25704ec))
* **charging_stations:** adding title and adding features for alternatives ([26751cd](https://github.com/tomtom-international/maps-sdk-js/commit/26751cd13e4997049afc20622ba291b522dc81f6))
* **charging_stations:** work in progress ([0b81da0](https://github.com/tomtom-international/maps-sdk-js/commit/0b81da0f8cae9f69728696c5f286e8f47594f3bc))
* **charging_stations:** work in progress ([a6f6aae](https://github.com/tomtom-international/maps-sdk-js/commit/a6f6aaec0b31cbc9e03ef7e6c9711385e8bb2b11))
* commit all possible doc changes in release-please ([525b22b](https://github.com/tomtom-international/maps-sdk-js/commit/525b22be6faea017d2147bf56e423dff3e53b27c))
* compress thumbnail images in examples ([b07139e](https://github.com/tomtom-international/maps-sdk-js/commit/b07139ead078dcd7f1f86f76195bf7d1928985db))
* **core:** improvin polygon feature typing ([295a48b](https://github.com/tomtom-international/maps-sdk-js/commit/295a48b67f7e92327bddd738c08c37974a778f6a))
* documentation structure ([ad61217](https://github.com/tomtom-international/maps-sdk-js/commit/ad612176e30c86ddbf34a5833d8c5bcb22317f2b))
* e2e test setup and misc improvements ([b23b540](https://github.com/tomtom-international/maps-sdk-js/commit/b23b5405c8ac476eef84dbf01d4f0575bcc101da))
* **EDXCE-282:** ensuring display waypoints always have feature IDs, which is needed for events and always good to have ([809ac21](https://github.com/tomtom-international/maps-sdk-js/commit/809ac21a057d2a12177aaf1e82b442b7697f6928))
* **EDXCE-282:** ensuring display waypoints always have feature IDs, which is needed for events and always good to have ([f8f692d](https://github.com/tomtom-international/maps-sdk-js/commit/f8f692d9778868c08a1a164bde8a7c6265194400))
* **edxce-337:** removing legacy tollRoad section type in favor of the new "toll" one, while keeping tollVignette even if not supported yet ([1e40ef9](https://github.com/tomtom-international/maps-sdk-js/commit/1e40ef9f6d1f7bf1f16925741c3177c294662d61))
* **EDXCE-89:** Adding TODO comments and updating a test ([94c252b](https://github.com/tomtom-international/maps-sdk-js/commit/94c252b0e4de730d8515ff11fd10e55518055c51))
* **EDXCE-89:** improvements ([7580948](https://github.com/tomtom-international/maps-sdk-js/commit/758094876ce144d909f69d467c864f0915b56dbd))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis improvements and some cleanup ([8346fe0](https://github.com/tomtom-international/maps-sdk-js/commit/8346fe0acdc59761d1d31634ccbea00feb47af87))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis, work in progress, added ldEV ([006c173](https://github.com/tomtom-international/maps-sdk-js/commit/006c1735b1ef0a486c7379392bb218d90856db22))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis, work in progress, fixing tests ([347f782](https://github.com/tomtom-international/maps-sdk-js/commit/347f78275851dd2444cd7da5fc1d91504840b0c1))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis, work in progress, lots of commented out code ([b802f80](https://github.com/tomtom-international/maps-sdk-js/commit/b802f80cd47881eb036b3f1e3f4f553507d161c8))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis, work in progress, reverting back vehicleRestricted ([5a0e5bf](https://github.com/tomtom-international/maps-sdk-js/commit/5a0e5bfffc4cc4e050b7aaa20972dce94120ff6a))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis, work in progress, support for new sections ([b3a9df6](https://github.com/tomtom-international/maps-sdk-js/commit/b3a9df67d74573507232c7e2265f9e290392337e))
* emit cjs builds for examples ([2fa1ab4](https://github.com/tomtom-international/maps-sdk-js/commit/2fa1ab4c7a9d103b36fbd7aed14d29538c07d46b))
* ensure appname is full repo with owner ([baf912f](https://github.com/tomtom-international/maps-sdk-js/commit/baf912f1fe983b7fb7c157b765a0a8d23b65de5f))
* error handling logic ([0b38424](https://github.com/tomtom-international/maps-sdk-js/commit/0b384241048610de73170cd86208b123c02c4137))
* fetch test after changing returned response format ([39d665b](https://github.com/tomtom-international/maps-sdk-js/commit/39d665bae3322023d8d855f40b625c1afc17f6dd))
* formatting html and css with biome ([5a9d7b3](https://github.com/tomtom-international/maps-sdk-js/commit/5a9d7b3893beda0dc84d539189f6db535c932c02))
* further reduce thumbnails in resolution and size ([f92d88f](https://github.com/tomtom-international/maps-sdk-js/commit/f92d88f40535c5421400322e24a4ed21436075be))
* **geometries:** small typing simplification / fix ([165b047](https://github.com/tomtom-international/maps-sdk-js/commit/165b0473a2fd2a84c1a49b748c13ab3d4830dd98))
* include only src folders for biome linting ([cf3ccc2](https://github.com/tomtom-international/maps-sdk-js/commit/cf3ccc25875295c86e2e8acfdf23a0fb89609f0c))
* keep sdk version in sync with docs ([e6c314d](https://github.com/tomtom-international/maps-sdk-js/commit/e6c314d5f5e9b81c3a80ac07b8e377a895575ee4))
* make all functions to be iife ([93ff690](https://github.com/tomtom-international/maps-sdk-js/commit/93ff690b948279485b2884846c2ad81549140d58))
* map features after breaking style changes ([a549821](https://github.com/tomtom-international/maps-sdk-js/commit/a5498215ff40fe0f0f74e7c41f4e81384bcdf864))
* **map:** fixing petrol station map poi mapping ([81f0e47](https://github.com/tomtom-international/maps-sdk-js/commit/81f0e477a382a3eb48d89493760f045afd82138c))
* **map:** hardening logic when styles and module changes are called quickly in succession ([9c56582](https://github.com/tomtom-international/maps-sdk-js/commit/9c56582a0fe6c9a2b17bee47787bf5c96bd1298e))
* **map:** hardening logic when styles and module changes are called quickly in succession ([756417a](https://github.com/tomtom-international/maps-sdk-js/commit/756417a7d1571965e299e74bd56cf871080ff423))
* **map:** hardening logic when styles and module changes are called quickly in succession ([50f74ec](https://github.com/tomtom-international/maps-sdk-js/commit/50f74ececdb524f5828a1cf9467a6d683d79786c))
* **map:** hotfix to allow supplying valid 0s as inputs ([7df0570](https://github.com/tomtom-international/maps-sdk-js/commit/7df0570007ba6c8528a0d37e0f8f394aabd2fc2a))
* **map:** moving summary bubble layer to the top ([7d5f143](https://github.com/tomtom-international/maps-sdk-js/commit/7d5f143ccf4e4b4c4e0874fd1cd08e5ec5805516))
* **map:** multiple fixes related to changing style vs events and showing geojson items ([2234fc0](https://github.com/tomtom-international/maps-sdk-js/commit/2234fc0bc2346aa5fdf865d6730150a80299a9cc))
* **map:** removing maxzoom for charging stations on route for now ([1183d1c](https://github.com/tomtom-international/maps-sdk-js/commit/1183d1c7827af2580a68a071eff07bcf341f5299))
* **map:** trying to allow for style data handler errors and still call the following ones. And minor misc fixes. ([57b8849](https://github.com/tomtom-international/maps-sdk-js/commit/57b88498e2240aafa86e40a1053d20074b8eeee5))
* **misc:** fixing tests and adding EV accessibility enum ([73cb76e](https://github.com/tomtom-international/maps-sdk-js/commit/73cb76e854dc9473dfff18b2838f1b3f02b83293))
* **misc:** removing incremental tsconfig setting to try to fix rollup bundling ([710e77c](https://github.com/tomtom-international/maps-sdk-js/commit/710e77c2eba0cd0b8e81cb16652095ba1d8aede6))
* missing maplibre css import ([2dc4c35](https://github.com/tomtom-international/maps-sdk-js/commit/2dc4c35a835ba042f37f5eb42eaf874593fdc90f))
* missing typedoc ([4f3e712](https://github.com/tomtom-international/maps-sdk-js/commit/4f3e7129467e32605064903c4c62e6a7b31a39f2))
* only update and commit docs to release branch before a release is created ([42c79ee](https://github.com/tomtom-international/maps-sdk-js/commit/42c79eeff23bc9352ce6fd4f200a108d4ed9f228))
* pin location ([41c3130](https://github.com/tomtom-international/maps-sdk-js/commit/41c3130c29bdf68f0f5f242e8fdce3a922a1aab5))
* **places:** attempting to mitigate strange glitch where showFeatures isn't updated in sdk geojson sources ([c93b24d](https://github.com/tomtom-international/maps-sdk-js/commit/c93b24d8306a4751387115c60bc375cfb149dde4))
* quick reductions of example titles ([431f64a](https://github.com/tomtom-international/maps-sdk-js/commit/431f64ab5e3d5df75c16ce4e9b9e6c4dc538ba27))
* release action ([2d52079](https://github.com/tomtom-international/maps-sdk-js/commit/2d52079a7b1af8eddcf307130b84929644f40f3b))
* release action ([3aff386](https://github.com/tomtom-international/maps-sdk-js/commit/3aff3861f2cd070dc4015776f5d294494862a579))
* remove redundant page title ([d9d677b](https://github.com/tomtom-international/maps-sdk-js/commit/d9d677b9014e6ba42eb3ef6bf1d3d1d785e65adf))
* rename project in veracode, blackduck scan workflows ([ed684e9](https://github.com/tomtom-international/maps-sdk-js/commit/ed684e94d9f3f1c2da36a5f337acb0ab7996517d))
* revert script accidental change ([3aa4ec5](https://github.com/tomtom-international/maps-sdk-js/commit/3aa4ec50085cfbdb3f9e3f5bdf89021c2504dbb3))
* **revgeo:** temporarily skipping flaky test, to be re-added later in a safer way ([6737bc1](https://github.com/tomtom-international/maps-sdk-js/commit/6737bc1632959e41dfbcdeff98559b8f5114a91d))
* **routing:** attempting to have the routing layers ready as soon as possible and then schedule loading images ([88bbcae](https://github.com/tomtom-international/maps-sdk-js/commit/88bbcaeff6d0dec57f0aa2607fc27d4a4c597f06))
* **routing:** correcting section type inputs ([fbf5aba](https://github.com/tomtom-international/maps-sdk-js/commit/fbf5aba070975f47beb0b5c3cd1b13839092bfb1))
* **routing:** fixing parsing of toll roads, hardening section parsing and improving position utils ([a5f3a73](https://github.com/tomtom-international/maps-sdk-js/commit/a5f3a73502e5e4a27c518e6d394c9d789d4b49b5))
* **routing:** fixing the mapping of instructions to path points ([b772b60](https://github.com/tomtom-international/maps-sdk-js/commit/b772b60aec80cbdca2eda84eff463260dac9014f))
* **routing:** fixing waypoints rendering tests ([83e36b8](https://github.com/tomtom-international/maps-sdk-js/commit/83e36b8ffd5b1e94ac88ea4ee59e9a255ec4cbd4))
* **routing:** hardening the display of instruction lines on the map ([4f4e965](https://github.com/tomtom-international/maps-sdk-js/commit/4f4e965c92a2cabd4af6d696866dbfce8a84e5f3))
* **routing:** hotfix to make route summary bubbles display more compact and mitigate unwanted word wrapping ([3cfd2ea](https://github.com/tomtom-international/maps-sdk-js/commit/3cfd2eaa4e8c1e6b11fc478939d8d696e56eb544))
* **routing:** hotfix to show ev charging stations above most other layers on routes ([87a9625](https://github.com/tomtom-international/maps-sdk-js/commit/87a96254a462962cd5a9e6bc0165aa69e155e8ae))
* **routing:** test stability hotfix ([c53dc51](https://github.com/tomtom-international/maps-sdk-js/commit/c53dc5183656c43f68430c397d5e33db3b852a04))
* **routing:** trying to fix ts check in tests project ([2751abf](https://github.com/tomtom-international/maps-sdk-js/commit/2751abf147b19cd60d2ee291334546c3af3c5f1e))
* schema test ([25c05a3](https://github.com/tomtom-international/maps-sdk-js/commit/25c05a373da4810b251c943cf62e3590a132ee1d))
* schemas and tests after upgrading ([e6426e0](https://github.com/tomtom-international/maps-sdk-js/commit/e6426e028b8b740aeec47740ecf792d3243fbaf3))
* **sdk:** flaky test mitigation ([5e0c4ca](https://github.com/tomtom-international/maps-sdk-js/commit/5e0c4ca87649c4950982fe6b92cadd1a4e40c88e))
* **sdk:** helping tests run better in ci cd ([fe79d4f](https://github.com/tomtom-international/maps-sdk-js/commit/fe79d4fa069b5229f1c98c3d4cf50205ffb6e525))
* **sdk:** trying to make the geojson modules more resilient to quick changes in succession ([0237c7c](https://github.com/tomtom-international/maps-sdk-js/commit/0237c7c8066ed62574a140f8a4a6154568970888))
* **sdk:** trying to stabilize tests in ci cd ([bcc4c34](https://github.com/tomtom-international/maps-sdk-js/commit/bcc4c34f9c1860ffb66887dc46281c3c27928167))
* **server:** attempt to skip loading the document object when in non-browser environments ([fc683ee](https://github.com/tomtom-international/maps-sdk-js/commit/fc683ee1a14838dac777c15a1a0c94164002cc5c))
* **server:** attempt to skip loading the document object when in non-browser environments ([41a9aa9](https://github.com/tomtom-international/maps-sdk-js/commit/41a9aa97c3d6c5b018a4a4e7347af5dc97341bbd))
* **server:** attempt to skip loading the document object when in non-browser environments ([04c4967](https://github.com/tomtom-international/maps-sdk-js/commit/04c4967bd98ccf00f540b740fdbe036f88df6b8a))
* **server:** attempt to skip loading the document object when in non-browser environments ([689696a](https://github.com/tomtom-international/maps-sdk-js/commit/689696a3be5b46911d1ecd1fa6e857c2532fbeac))
* **services:** hot fixing unstable routing integration test ([6fb98fb](https://github.com/tomtom-international/maps-sdk-js/commit/6fb98fb3840554fdd449614f75b89b829dfeef36))
* supporting receiving undefined connectors for charging stations ([4393944](https://github.com/tomtom-international/maps-sdk-js/commit/43939448ceff59c91c9a45b05c3b049f87e1aa2d))
* **tests:** enabling map mocked tests with local asset mocking ([bb099d3](https://github.com/tomtom-international/maps-sdk-js/commit/bb099d367469f9cc70dad2a70d9aab52923d8cb0))
* **tests:** enabling puppeteer tests to still run while SDK imports trigger detection of document object due to embedded sdk assets ([fefc66e](https://github.com/tomtom-international/maps-sdk-js/commit/fefc66e9131fae4b984a043a325fbb435e62c3b8))
* **tests:** hardening POI map tests, removing unnecessary and misleading assertion, since changing POI filters doesn't mean number of visible POIs always changes ([aa86082](https://github.com/tomtom-international/maps-sdk-js/commit/aa8608213f500055e1a8c7baf52d968e11f9bd87))
* **tests:** stabilizing traffic incident tests for situations where we only have road closures available ([3d77714](https://github.com/tomtom-international/maps-sdk-js/commit/3d77714a9e9081563429c3b8ccd195efdb10700f))
* **tests:** updating search test IDs after another search api Orbis migration ([b9bcd59](https://github.com/tomtom-international/maps-sdk-js/commit/b9bcd596df7f149213dc70b264a7907cf5b652ec))
* **test:** trying to stabilize test ([f8111ff](https://github.com/tomtom-international/maps-sdk-js/commit/f8111ffdca8c29d150ea1cce6b7a25f7283dac0a))
* **test:** trying to stabilize test ([93fcb2f](https://github.com/tomtom-international/maps-sdk-js/commit/93fcb2f348b37d24e7d8c755a50117c12a804ece))
* **traffic:** removing confusing unused type ([25adb33](https://github.com/tomtom-international/maps-sdk-js/commit/25adb331a87fbf4c6c5d1c140478cc72f33191b7))
* undo unwanted vite config changes ([000a629](https://github.com/tomtom-international/maps-sdk-js/commit/000a62926005da2b7965b157d0f30bec9839876a))
* update all developer portal links to docs portal ([55bf93c](https://github.com/tomtom-international/maps-sdk-js/commit/55bf93cce4467a744bc02a9a5c8b34cad0748ad6))
* update blackduck runner ([74e88ce](https://github.com/tomtom-international/maps-sdk-js/commit/74e88ce79c3baae7764435dc3d4332fd73b7f603))
* update image to thumbnail property ([ab544f4](https://github.com/tomtom-international/maps-sdk-js/commit/ab544f4fed655589f2a2222a82832fd4875be104))
* update prop in release please config ([cae9e71](https://github.com/tomtom-international/maps-sdk-js/commit/cae9e717a03f55cb4005ac0e864e99bcc6e5193b))
* update tags for examples ([9228c4c](https://github.com/tomtom-international/maps-sdk-js/commit/9228c4ca8f6a315980de078aab18fb8834fabe90))
* updated version of blackduck action ([8be1b34](https://github.com/tomtom-international/maps-sdk-js/commit/8be1b3459d4d3d47840e39f25ac373cf3b92337e))
* upgrade blackduck and python versions ([5509d3c](https://github.com/tomtom-international/maps-sdk-js/commit/5509d3c63a6606ed1975f28418943d6666c56306))
* use bash for script executions for compatibility ([8d6bf4f](https://github.com/tomtom-international/maps-sdk-js/commit/8d6bf4f8d577917abd6562789f011377310c6b3c))
* use existing examples package json ([ea28a01](https://github.com/tomtom-international/maps-sdk-js/commit/ea28a014e4bdc321f5bb4771845d66eeb5a9c804))
* workflow calling wrong tests ([037c89c](https://github.com/tomtom-international/maps-sdk-js/commit/037c89cb6e39a4fb541991a366724ea49b164a96))

## [0.20.0](https://github.com/tomtom-international/maps-sdk-js/compare/v0.19.0...v0.20.0) (2025-10-16)


### Features

* add geometry tag ([eb2ba44](https://github.com/tomtom-international/maps-sdk-js/commit/eb2ba446a5b7a736db511f6587997d30a967678a))
* add new basic examples ([4beb2b4](https://github.com/tomtom-international/maps-sdk-js/commit/4beb2b4a1c2a64d57e5ce3fe7335d5c1982e6e74))


### Bug Fixes

* update tags for examples ([6dc5efb](https://github.com/tomtom-international/maps-sdk-js/commit/6dc5efbbbe25cbca2515bc083fae54b37b5cde09))

## [0.19.0](https://github.com/tomtom-international/maps-sdk-js/compare/v0.18.0...v0.19.0) (2025-10-13)


### Features

* simplify example tags and groupings ([591d23e](https://github.com/tomtom-international/maps-sdk-js/commit/591d23e842a2f961c22a9c8e0e6f2fbb9618f137))


### Bug Fixes

* remove redundant page title ([525cfbc](https://github.com/tomtom-international/maps-sdk-js/commit/525cfbcd85d3e26915d14c87a2c44e6357e4261d))

## [0.18.0](https://github.com/tomtom-international/maps-sdk-js/compare/v0.17.0...v0.18.0) (2025-10-10)


### Features

* basic ldevr example ([fb0c48f](https://github.com/tomtom-international/maps-sdk-js/commit/fb0c48f4b89db0811d0aeb3417999fe95dbd2188))
* pin improvements including ev charging stops ([4faf282](https://github.com/tomtom-international/maps-sdk-js/commit/4faf282e7173a622e4694b1ef07739636c64bbba))


### Bug Fixes

* blackduck runner ([fd26738](https://github.com/tomtom-international/maps-sdk-js/commit/fd26738b20eaaffc652c766087d0393c2493d577))
* update all developer portal links to docs portal ([0412067](https://github.com/tomtom-international/maps-sdk-js/commit/0412067b4d7a8961cc7e66ef8d16a9e7011cf015))

## [0.17.0](https://github.com/tomtom-international/maps-sdk-js/compare/v0.16.0...v0.17.0) (2025-10-08)


### Features

* move constants from docs portal to examples ([18d5cc5](https://github.com/tomtom-international/maps-sdk-js/commit/18d5cc5ce0a5021870c252f541f54c68f5f6c684))
* restructure tags filter ([b42db70](https://github.com/tomtom-international/maps-sdk-js/commit/b42db7059d7a03f163634a50b99f47f9b55c688a))


### Bug Fixes

* emit cjs builds for examples ([52c0f33](https://github.com/tomtom-international/maps-sdk-js/commit/52c0f33e4fdc76122a7b812b1f50a4678b362cd7))

## [0.16.0](https://github.com/tomtom-international/maps-sdk-js/compare/v0.15.1...v0.16.0) (2025-10-07)


### Features

* switch from published to standard styles for clarity ([d996190](https://github.com/tomtom-international/maps-sdk-js/commit/d996190aa6e8ac49c0778812bfad6063b30324f9))


### Bug Fixes

* quick reductions of example titles ([01e63e2](https://github.com/tomtom-international/maps-sdk-js/commit/01e63e2924b711f2b719161143bb2ccfc441a9d1))
* supporting receiving undefined connectors for charging stations ([0a36101](https://github.com/tomtom-international/maps-sdk-js/commit/0a36101fab3f818e92e8aafce241ff66d2f111d8))
* update blackduck runner ([c7c79a7](https://github.com/tomtom-international/maps-sdk-js/commit/c7c79a7acd5a07528a414ef068331c38e395c529))

## [0.15.1](https://github.com/tomtom-international/maps-sdk-js/compare/v0.15.0...v0.15.1) (2025-10-03)


### Bug Fixes

* further reduce thumbnails in resolution and size ([2e75343](https://github.com/tomtom-international/maps-sdk-js/commit/2e7534337d19b26eb987706d323b77cc11db0810))

## [0.15.0](https://github.com/tomtom-international/maps-sdk-js/compare/v0.14.0...v0.15.0) (2025-10-03)


### Features

* move all html and css files from examples under src ([0abeaf0](https://github.com/tomtom-international/maps-sdk-js/commit/0abeaf01f45c4bea0e6a45f8c66e2268f0c66fc7))


### Bug Fixes

* compress thumbnail images in examples ([f356de7](https://github.com/tomtom-international/maps-sdk-js/commit/f356de70afc82127d81de6e2b3ca535f4a6b97cd))
* only update and commit docs to release branch before a release is created ([ebb1a98](https://github.com/tomtom-international/maps-sdk-js/commit/ebb1a98ca748e5f0dcfae42453aa0d91596dc8f3))
* update image to thumbnail property ([6fb448c](https://github.com/tomtom-international/maps-sdk-js/commit/6fb448cc8e31310eb747a641be6ce5606e20abd4))

## [0.14.0](https://github.com/tomtom-international/maps-sdk-js/compare/v0.13.0...v0.14.0) (2025-09-30)


### Features

* add missing maplibre css imports and change colors in tune with tomtom brand ([cf5335d](https://github.com/tomtom-international/maps-sdk-js/commit/cf5335dd105e8ae239a512bd559fb3d5248c75b9))
* improve css imports ([3c9bac7](https://github.com/tomtom-international/maps-sdk-js/commit/3c9bac70a2acf201d00b90cc24e940beeeeeb95a))
* include examples page in docs portal page as content ([0d0f2fa](https://github.com/tomtom-international/maps-sdk-js/commit/0d0f2fa21622d03f711f62ac35b7301d88eca8d6))
* remove async wrappers from examples by using esnext on tsconfig ([d6a2504](https://github.com/tomtom-international/maps-sdk-js/commit/d6a2504708a440c2a788797361008dec17265d24))
* use release-please to bump examples version ([5ae4569](https://github.com/tomtom-international/maps-sdk-js/commit/5ae4569302a8c8fe2fab78419a2690b6cbfd472e))


### Bug Fixes

* commit all possible doc changes in release-please ([fcb7956](https://github.com/tomtom-international/maps-sdk-js/commit/fcb79569fcc8ba945033a01c2c3c9399cf0e9424))
* keep sdk version in sync with docs ([5a2e5f6](https://github.com/tomtom-international/maps-sdk-js/commit/5a2e5f61922c4fec0d42be1be6f225f3ac7ae5dd))
* missing maplibre css import ([0e3f025](https://github.com/tomtom-international/maps-sdk-js/commit/0e3f025130f8cf07b7e359672f3a6826c402ae49))
* update prop in release please config ([1464674](https://github.com/tomtom-international/maps-sdk-js/commit/1464674e5a79959e6341c3cde58319cfa52f09f0))
* use bash for script executions for compatibility ([817d0c7](https://github.com/tomtom-international/maps-sdk-js/commit/817d0c72f9328bd489ea39ddcc7cb006022fac87))

## [0.13.0](https://github.com/tomtom-international/maps-sdk-js/compare/v0.12.1...v0.13.0) (2025-09-25)


### Features

* add script to copy examples dist ([581033c](https://github.com/tomtom-international/maps-sdk-js/commit/581033c0538cf3ffd4f249d0c0861fe5c5f66f8d))
* use version of the root package.json ([cbc186d](https://github.com/tomtom-international/maps-sdk-js/commit/cbc186d8719eee12e85941e5977d068a4061b9a2))


### Bug Fixes

* use existing examples package json ([a28bd86](https://github.com/tomtom-international/maps-sdk-js/commit/a28bd866f03f4474da3f114ea1095b3a17215038))

## [0.12.1](https://github.com/tomtom-international/maps-sdk-js/compare/v0.12.0...v0.12.1) (2025-09-23)


### Bug Fixes

* add missing typedoc ([41140b5](https://github.com/tomtom-international/maps-sdk-js/commit/41140b5939af5595361405a0eacdfd69faed576a))
* formatting html and css with biome ([e49b4c5](https://github.com/tomtom-international/maps-sdk-js/commit/e49b4c51fa579c153eb84d0aaca24741cd5742b3))

## [0.12.0](https://github.com/tomtom-international/maps-sdk-js/compare/v0.11.0...v0.12.0) (2025-09-23)


### Features

* do not link to source files from API reference since we consider them implementation details ([9530052](https://github.com/tomtom-international/maps-sdk-js/commit/9530052103b3bf390ba34340ea6d1234f6a7d414))


### Bug Fixes

* api reference workflow ([89c5d32](https://github.com/tomtom-international/maps-sdk-js/commit/89c5d32a8929f65971df25a842aad120c7622820))
* api reference workflow to react better to triggers ([6948731](https://github.com/tomtom-international/maps-sdk-js/commit/69487316133cd81b26e8067f580da0866e6bac99))
* api reference workflow to react better to triggers ([fb88afd](https://github.com/tomtom-international/maps-sdk-js/commit/fb88afdcab6ffb7503ef1dd5fecd891523bbb875))
* missing typedoc ([7dfd9d2](https://github.com/tomtom-international/maps-sdk-js/commit/7dfd9d2abd7c3aeb50807ae69b055bcb933a86df))

## [0.11.0](https://github.com/tomtom-international/maps-sdk-js/compare/v0.10.0...v0.11.0) (2025-09-23)


### Features

* typedoc improvements ([500d95f](https://github.com/tomtom-international/maps-sdk-js/commit/500d95f539a6c03d4d7673402e4eaa9bbd37ad28))

## [0.10.0](https://github.com/tomtom-international/maps-sdk-js/compare/v0.9.3...v0.10.0) (2025-09-23)


### Features

* api reference integration in repo and npm ([45ae8aa](https://github.com/tomtom-international/maps-sdk-js/commit/45ae8aa6b82ad6a41f19d996574022abface78ae))
* api reference integration in repo and npm ([81b1a79](https://github.com/tomtom-international/maps-sdk-js/commit/81b1a7976efb55e428c0c4d6f87571b316b9d817))
* api reference integration in repo and npm ([626a03a](https://github.com/tomtom-international/maps-sdk-js/commit/626a03a3c9cd988093e6fef5dbe9aa12cde4305c))
* api reference integration in repo and npm ([3a98991](https://github.com/tomtom-international/maps-sdk-js/commit/3a98991f14268584321d61cd4c177c48658c255b))
* api reference integration in repo and npm ([56fbae5](https://github.com/tomtom-international/maps-sdk-js/commit/56fbae5e88fdc91bb7ab89991464b0bcd5b6d200))
* api reference integration in repo and npm ([c197bfb](https://github.com/tomtom-international/maps-sdk-js/commit/c197bfb37b7f609c6a84686a44f3d2c83b521936))
* api reference integration in repo and npm ([1393975](https://github.com/tomtom-international/maps-sdk-js/commit/13939752f735b068fad7bbcc86704f36f8756043))
* api reference landing page with a link to the full api reference pages ([4e2d7e5](https://github.com/tomtom-international/maps-sdk-js/commit/4e2d7e518dc2c42addb9cdf3b6748ae8642c637c))
* cleanup and improve example mdx content ([7be5c95](https://github.com/tomtom-international/maps-sdk-js/commit/7be5c955ec3069bca6d8815ef52fbcce87cd42f2))
* documentation directory improvements ([c0bb624](https://github.com/tomtom-international/maps-sdk-js/commit/c0bb62433d9f634f6e847d2d713795c9cebcf56a))
* documentation improvements first iteration using ai agent ([893bf4e](https://github.com/tomtom-international/maps-sdk-js/commit/893bf4e54735ba76cfe10c89322a262f134b38b6))
* remove api reference from navigation file ([6d236da](https://github.com/tomtom-international/maps-sdk-js/commit/6d236da355ffd731743ecc3ef08198b0e48b21d9))


### Bug Fixes

* api reference and docs navigation generation ([7f0a848](https://github.com/tomtom-international/maps-sdk-js/commit/7f0a848085d54e3090aadaf7892d0d081d01201e))
* documentation structure ([74fa765](https://github.com/tomtom-international/maps-sdk-js/commit/74fa7652a436d950a6aec2c00d9a18799372a3e9))
* e2e test setup and misc improvements ([aae4081](https://github.com/tomtom-international/maps-sdk-js/commit/aae408116d98580381fd9e00310397792ee85266))
* make all functions to be iife ([acd34fa](https://github.com/tomtom-international/maps-sdk-js/commit/acd34fa53fa32a5abc02f2e8b7b53cb61359d4ac))
* undo unwanted vite config changes ([fe30437](https://github.com/tomtom-international/maps-sdk-js/commit/fe30437d775b5f4949389079b57b143791b086ab))
* workflow calling wrong tests ([a627b4f](https://github.com/tomtom-international/maps-sdk-js/commit/a627b4f01724c6fb2ee2fe75cfe77bfc4283619d))

## [0.9.3](https://github.com/tomtom-international/maps-sdk-js/compare/v0.9.2...v0.9.3) (2025-09-09)


### Bug Fixes

* branch cleanup action ([721ebeb](https://github.com/tomtom-international/maps-sdk-js/commit/721ebeb455124237f7e1837b957e7cf9460b62c5))

## [0.9.2](https://github.com/tomtom-international/maps-sdk-js/compare/v0.9.1...v0.9.2) (2025-09-09)


### Bug Fixes

* release action ([37938c2](https://github.com/tomtom-international/maps-sdk-js/commit/37938c248b5b89ee2de8269a8d5b117050e270e8))

## [0.9.1](https://github.com/tomtom-international/maps-sdk-js/compare/v0.9.0...v0.9.1) (2025-09-09)


### Bug Fixes

* release action ([252fc57](https://github.com/tomtom-international/maps-sdk-js/commit/252fc5736e65e639c85bff4500a9534a0bcdd3ab))

## [0.9.0](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.24...v0.9.0) (2025-09-09)


### Features

* bring rest of relevant examples into monorepo ([fe24553](https://github.com/tomtom-international/maps-sdk-js/commit/fe2455387c63b84d9198b470f94c99c8459e0101))
* improve how places are rendered ([6c09bc7](https://github.com/tomtom-international/maps-sdk-js/commit/6c09bc7a16e67846882888a9781e41110cc1c079))
* restoring pins on the map based on new assets ([74b0e95](https://github.com/tomtom-international/maps-sdk-js/commit/74b0e950a872767f9797dc5b9c0c28f8967698ef))
* switch to monorepo with examples ([8f67ff3](https://github.com/tomtom-international/maps-sdk-js/commit/8f67ff33569c1ab01f798c96add2878611e75407))


### Bug Fixes

* attempt to mitigate style changes vs geojson race conditions ([f277779](https://github.com/tomtom-international/maps-sdk-js/commit/f27777912577a40fd6d350e8c0d551fce283e84e))

### [0.8.24](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.23...v0.8.24) (2025-07-24)


### Bug Fixes

* pin location ([cd002e3](https://github.com/tomtom-international/maps-sdk-js/commit/cd002e322643d73307d61ff5b61e7e8886c4a60e))

### [0.8.23](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.22...v0.8.23) (2025-07-24)


### Bug Fixes

* schema test ([62a1bbc](https://github.com/tomtom-international/maps-sdk-js/commit/62a1bbc22ac575cd259a557b6a1d867ced74726b))

### [0.8.22](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.21...v0.8.22) (2025-07-22)

### [0.8.21](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.20...v0.8.21) (2025-07-08)


### Bug Fixes

* added bdtrc file ([78094a6](https://github.com/tomtom-international/maps-sdk-js/commit/78094a6edbcddb8dd586fede8a61fa173d7f7ab6))
* map features after breaking style changes ([a4e2d96](https://github.com/tomtom-international/maps-sdk-js/commit/a4e2d96982aa195dcf01278cba0f641b6975cd8c))
* revert script accidental change ([b26b598](https://github.com/tomtom-international/maps-sdk-js/commit/b26b59897e81448fff00c4255471794dabe95189))
* schemas and tests after upgrading ([57dde5e](https://github.com/tomtom-international/maps-sdk-js/commit/57dde5ee3ab20495f02dc196b2e6d9252474eca3))
* updated version of blackduck action ([7cd9888](https://github.com/tomtom-international/maps-sdk-js/commit/7cd988812c4ee6328657ea556fc06119edf7b2ff))

### [0.8.20](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.19...v0.8.20) (2024-06-20)


### Features

* **routing:** adding basic entry points support when calculating routes and displaying waypoints ([0feadb4](https://github.com/tomtom-international/maps-sdk-js/commit/0feadb4ce909d7c7f3b2bed6da14a26a6a704343))

### [0.8.19](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.18...v0.8.19) (2024-06-13)


### Bug Fixes

* **map:** moving summary bubble layer to the top ([5928ed9](https://github.com/tomtom-international/maps-sdk-js/commit/5928ed9cc0c42221ec2a4885e7f94e744643a78f))

### [0.8.18](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.17...v0.8.18) (2024-06-07)

### [0.8.17](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.16...v0.8.17) (2024-06-06)


### Bug Fixes

* **edxce-337:** removing legacy tollRoad section type in favor of the new "toll" one, while keeping tollVignette even if not supported yet ([5da658c](https://github.com/tomtom-international/maps-sdk-js/commit/5da658cd81e55611696a6c4c712a818b3a846acf))

### [0.8.16](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.15...v0.8.16) (2024-05-30)

### [0.8.15](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.14...v0.8.15) (2024-05-29)


### Features

* pass info about response status to onAPIResponse ([21f5a86](https://github.com/tomtom-international/maps-sdk-js/commit/21f5a86ac66dbd1a302352276d6ffef08235bf5c))


### Bug Fixes

* error handling logic ([3abd700](https://github.com/tomtom-international/maps-sdk-js/commit/3abd700cf0981fee7bda49333ad276ecdbc4cba7))
* fetch test after changing returned response format ([0c9ee1c](https://github.com/tomtom-international/maps-sdk-js/commit/0c9ee1c581ea59d72be4c2489af002258e1dcd50))

### [0.8.14](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.13...v0.8.14) (2024-05-16)

### [0.8.13](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.12...v0.8.13) (2024-05-13)


### Bug Fixes

* **tests:** updating search test IDs after another search api Orbis migration ([d4bc67f](https://github.com/tomtom-international/maps-sdk-js/commit/d4bc67f1f636bbea5a0b1f2904061b91b7201f35))

### [0.8.12](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.11...v0.8.12) (2024-05-06)

### [0.8.11](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.10...v0.8.11) (2024-05-06)

### [0.8.10](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.9...v0.8.10) (2024-05-06)


### Features

* **EDXCE-275-EDXCE-282:** upgrading to maplibre 4.2.0 with disabled pending tiles canceling, adding waypoint event tests, and splitting events tests ([d981c25](https://github.com/tomtom-international/maps-sdk-js/commit/d981c259c6bab72b1a6c02fdee201184ada63be2))

### [0.8.9](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.8...v0.8.9) (2024-05-06)


### Features

* **EDXCE-275-EDXCE-282:** upgrading to maplibre 4.2.0 with disabled pending tiles canceling, adding waypoint event tests, and splitting events tests ([8e361df](https://github.com/tomtom-international/maps-sdk-js/commit/8e361df1ec2da225bc6bc3984ad879f293a6c4f0))

### [0.8.8](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.7...v0.8.8) (2024-04-30)


### Bug Fixes

* **routing:** fixing waypoints rendering tests ([c203cde](https://github.com/tomtom-international/maps-sdk-js/commit/c203cde93cc43e1929edf2685ca5dd2df815a0bd))

### [0.8.7](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.6...v0.8.7) (2024-04-30)


### Bug Fixes

* **revgeo:** temporarily skipping flaky test, to be re-added later in a safer way ([ba6e131](https://github.com/tomtom-international/maps-sdk-js/commit/ba6e131351299a7823320fb9e64b82d6a28e844d))

### [0.8.6](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.5...v0.8.6) (2024-04-29)

### [0.8.5](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.4...v0.8.5) (2024-04-29)


### Bug Fixes

* **EDXCE-282:** ensuring display waypoints always have feature IDs, which is needed for events and always good to have ([bd06a8d](https://github.com/tomtom-international/maps-sdk-js/commit/bd06a8d26115354bceeaf2d38839b01d23e04c42))

### [0.8.4](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.3...v0.8.4) (2024-04-29)


### Bug Fixes

* **EDXCE-282:** ensuring display waypoints always have feature IDs, which is needed for events and always good to have ([cc8b5a5](https://github.com/tomtom-international/maps-sdk-js/commit/cc8b5a544fceb73c8aeb05c8003aba5889d02063))

### [0.8.3](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.2...v0.8.3) (2024-04-27)


### Bug Fixes

* **tests:** stabilizing traffic incident tests for situations where we only have road closures available ([d618e83](https://github.com/tomtom-international/maps-sdk-js/commit/d618e834bc0f7e369bcbbaae6676ae46b4f14daf))

### [0.8.2](https://github.com/tomtom-international/maps-sdk-js/compare/v0.8.1...v0.8.2) (2024-04-27)


### Bug Fixes

* **tests:** hardening POI map tests, removing unnecessary and misleading assertion, since changing POI filters doesn't mean number of visible POIs always changes ([9e4be40](https://github.com/tomtom-international/maps-sdk-js/commit/9e4be40658e70eaf925c8a1958a58aeb5b2c5373))

### [0.8.1](https://github.com/tomtom-international/maps-sdk-js/compare/v0.7.9...v0.8.1) (2024-04-26)


### Features

* **EDXCE-266:** migrating ev charging station by id to new orbis endpoint, with included refactoring ([75155f4](https://github.com/tomtom-international/maps-sdk-js/commit/75155f43a9646a4f1b97774b3bab9c1414a8816a))

### [0.7.9](https://github.com/tomtom-international/maps-sdk-js/compare/v0.7.8...v0.7.9) (2024-04-23)


### Bug Fixes

* **routing:** hotfix to make route summary bubbles display more compact and mitigate unwanted word wrapping ([730e367](https://github.com/tomtom-international/maps-sdk-js/commit/730e3671d082986be080bf1d39af56cb04f48489))

### [0.7.8](https://github.com/tomtom-international/maps-sdk-js/compare/v0.7.7...v0.7.8) (2024-04-23)


### Bug Fixes

* **routing:** hotfix to show ev charging stations above most other layers on routes ([399d49b](https://github.com/tomtom-international/maps-sdk-js/commit/399d49bfbe53375204cb94dee53027b159fae670))

### [0.7.7](https://github.com/tomtom-international/maps-sdk-js/compare/v0.7.6...v0.7.7) (2024-04-22)


### Features

* **units:** enabling new global and local configurations for distance and time display units ([5dc489c](https://github.com/tomtom-international/maps-sdk-js/commit/5dc489c2ac51b8d6f6561024147952749f5d89f5))

### [0.7.6](https://github.com/tomtom-international/maps-sdk-js/compare/v0.7.5...v0.7.6) (2024-04-22)


### Features

* **units:** enabling new global and local configurations for distance and time display units ([85ca805](https://github.com/tomtom-international/maps-sdk-js/commit/85ca8054677651dfac97e316b7fc7d2ba26d16d1))


### Bug Fixes

* add .netrc file needed for blackduck scan ([c2cb1f2](https://github.com/tomtom-international/maps-sdk-js/commit/c2cb1f2afc771101c2061b1289bc7de515cbfb27))
* add install step ([0e12c32](https://github.com/tomtom-international/maps-sdk-js/commit/0e12c32f387d198fac88a39e2f2e2e6627cacf15))
* upgrade blackduck and python versions ([b637058](https://github.com/tomtom-international/maps-sdk-js/commit/b637058b34cb3aa3208012cbfae1f4806679f88b))

### [0.7.5](https://github.com/tomtom-international/maps-sdk-js/compare/v0.7.3...v0.7.5) (2024-04-18)

### [0.7.4](https://github.com/tomtom-international/maps-sdk-js/compare/v0.7.3...v0.7.4) (2024-04-18)

### [0.7.3](https://github.com/tomtom-international/maps-sdk-js/compare/v0.7.2...v0.7.3) (2024-04-09)

### [0.7.2](https://github.com/tomtom-international/maps-sdk-js/compare/v0.7.1...v0.7.2) (2024-04-04)

### [0.7.1](https://github.com/tomtom-international/maps-sdk-js/compare/v0.6.16...v0.7.1) (2024-04-04)

### [0.6.16](https://github.com/tomtom-international/maps-sdk-js/compare/v0.6.15...v0.6.16) (2024-04-04)


### Features

* **EDXCE-118:** switching places services to Orbis ([122f3b4](https://github.com/tomtom-international/maps-sdk-js/commit/122f3b4ab1bad28fdf5c8e57f37f36d7e5f6131a))

### [0.6.15](https://github.com/tomtom-international/maps-sdk-js/compare/v0.6.14...v0.6.15) (2024-04-02)


### Features

* **routing:** improving types ([354c8cf](https://github.com/tomtom-international/maps-sdk-js/commit/354c8cfd74fc5931486d2d6feb43531db6a22f21))

### [0.6.14](https://github.com/tomtom-international/maps-sdk-js/compare/v0.6.13...v0.6.14) (2024-03-22)


### Bug Fixes

* **routing:** correcting section type inputs ([c1879db](https://github.com/tomtom-international/maps-sdk-js/commit/c1879db871ea95ad17e545fdcdfaad07c333a782))

### [0.6.13](https://github.com/tomtom-international/maps-sdk-js/compare/v0.6.12...v0.6.13) (2024-03-21)


### Features

* **routing:** updated local svg-based waypoint assets and route line colors ([662aa75](https://github.com/tomtom-international/maps-sdk-js/commit/662aa752abfbfac9f34918deae1820d7ba3abd00))

### [0.6.12](https://github.com/tomtom-international/maps-sdk-js/compare/v0.6.11...v0.6.12) (2024-03-19)


### Features

* **services:** calling onAPIRequest also when APIs respond with errors ([eb08672](https://github.com/tomtom-international/maps-sdk-js/commit/eb08672cbcd9be9c04e362e2223a0003d8c0f34c))

### [0.6.11](https://github.com/tomtom-international/maps-sdk-js/compare/v0.6.10...v0.6.11) (2024-03-18)


### Bug Fixes

* **routing:** attempting to have the routing layers ready as soon as possible and then schedule loading images ([b09649a](https://github.com/tomtom-international/maps-sdk-js/commit/b09649af077cafee423ec6f647a2cbcafbd50f71))

### [0.6.10](https://github.com/tomtom-international/maps-sdk-js/compare/v0.6.9...v0.6.10) (2024-03-18)

### [0.6.9](https://github.com/tomtom-international/maps-sdk-js/compare/v0.6.8...v0.6.9) (2024-03-18)


### Features

* **routing:** allowing for default 0 value in maxAlternatives for convenience ([58da1ea](https://github.com/tomtom-international/maps-sdk-js/commit/58da1eaa230a2b4385d0d52b347b1385c4db0efc))

### [0.6.8](https://github.com/tomtom-international/maps-sdk-js/compare/v0.6.7...v0.6.8) (2024-03-18)


### Features

* **map:** default attribution control showing map copyrights ([e79a4c2](https://github.com/tomtom-international/maps-sdk-js/commit/e79a4c27c85f2cad12b741d8844dfdf70e018bc4))


### Bug Fixes

* **routing:** test stability hotfix ([c1ba464](https://github.com/tomtom-international/maps-sdk-js/commit/c1ba464edb6249867350b422702d8aa888c01e5d))

### [0.6.7](https://github.com/tomtom-international/maps-sdk-js/compare/v0.6.6...v0.6.7) (2024-03-14)


### Bug Fixes

* **traffic:** removing confusing unused type ([f566a98](https://github.com/tomtom-international/maps-sdk-js/commit/f566a9881ba1066e967f9b26c9efb5ea7454d3d7))

### [0.6.6](https://github.com/tomtom-international/maps-sdk-js/compare/v0.6.5...v0.6.6) (2024-03-13)


### Bug Fixes

* **routing:** fixing parsing of toll roads, hardening section parsing and improving position utils ([1de7738](https://github.com/tomtom-international/maps-sdk-js/commit/1de773824cc3c597c52a9021ad5b660788be04ad))

### [0.6.5](https://github.com/tomtom-international/maps-sdk-js/compare/v0.6.4...v0.6.5) (2024-03-11)

### [0.6.4](https://github.com/tomtom-international/maps-sdk-js/compare/v0.6.3...v0.6.4) (2024-03-11)


### Features

* **routing:** migrating to orbis routing v2 ([b5ec0f2](https://github.com/tomtom-international/maps-sdk-js/commit/b5ec0f2f26ea035e3d1c5486d5aa3538723a17d0))

### [0.6.3](https://github.com/tomtom-international/maps-sdk-js/compare/v0.6.2...v0.6.3) (2024-03-08)

### [0.6.2](https://github.com/tomtom-international/maps-sdk-js/compare/v0.6.1...v0.6.2) (2024-03-08)


### Features

* **routing:** automatically updating summary bubbles if distance units config is changed ([180f307](https://github.com/tomtom-international/maps-sdk-js/commit/180f307aad77e349a1ed3e5b3bb34a048fb9ced5))

### [0.6.1](https://github.com/tomtom-international/maps-sdk-js/compare/v0.5.7...v0.6.1) (2024-03-08)

### [0.5.7](https://github.com/tomtom-international/maps-sdk-js/compare/v0.5.6...v0.5.7) (2024-03-08)


### Features

* **routing:** first iteration for displaying bubbles with route summaries on the map ([be2cc3a](https://github.com/tomtom-international/maps-sdk-js/commit/be2cc3a8a4d163481f8b43b11172e0065d65546b))

### [0.5.6](https://github.com/tomtom-international/maps-sdk-js/compare/v0.5.5...v0.5.6) (2024-02-27)


### Bug Fixes

* **routing:** fixing the mapping of instructions to path points ([72330f5](https://github.com/tomtom-international/maps-sdk-js/commit/72330f5eaad229b74bcf5b76152807505a92dc27))

### [0.5.5](https://github.com/tomtom-international/maps-sdk-js/compare/v0.5.4...v0.5.5) (2024-02-23)


### Bug Fixes

* **server:** attempt to skip loading the document object when in non-browser environments ([bb4d124](https://github.com/tomtom-international/maps-sdk-js/commit/bb4d124d6fef8c74eab87b29ee102d09e47ebdff))

### [0.5.4](https://github.com/tomtom-international/maps-sdk-js/compare/v0.5.3...v0.5.4) (2024-02-23)


### Bug Fixes

* **server:** attempt to skip loading the document object when in non-browser environments ([1fd9f9a](https://github.com/tomtom-international/maps-sdk-js/commit/1fd9f9a21aed3228e60f53680fb763d357a026b8))
* **server:** attempt to skip loading the document object when in non-browser environments ([4b1cf6c](https://github.com/tomtom-international/maps-sdk-js/commit/4b1cf6ce47b6b64a5981e0c1c2e6caed13c455a3))

### [0.5.3](https://github.com/tomtom-international/maps-sdk-js/compare/v0.5.2...v0.5.3) (2024-02-23)


### Bug Fixes

* **server:** attempt to skip loading the document object when in non-browser environments ([58cec40](https://github.com/tomtom-international/maps-sdk-js/commit/58cec40fb1c80a55f1f67e5f2f738790773e332c))

### [0.5.2](https://github.com/tomtom-international/maps-sdk-js/compare/v0.5.1...v0.5.2) (2024-02-23)


### Features

* run blackduck workflow on PR and main pushes, populate result in a PR comment ([59edf06](https://github.com/tomtom-international/maps-sdk-js/commit/59edf06ce239d74e3f6cda3830e8a44db44332fd))
* **services:** exporting fetch types to facilitate overrides and api monitoring by clients ([9f01231](https://github.com/tomtom-international/maps-sdk-js/commit/9f0123152231b6c7e257dc85dfdff8318027f37b))

### [0.5.1](https://github.com/tomtom-international/maps-sdk-js/compare/v0.4.2...v0.5.1) (2024-02-14)


### Features

* **services:** removing axios and replacing it with fetch, together with nodejs 21 support ([2cf7e5d](https://github.com/tomtom-international/maps-sdk-js/commit/2cf7e5dd00340ee2e71d835cef93bf3b5de87296))

### [0.4.2](https://github.com/tomtom-international/maps-sdk-js/compare/v0.4.1...v0.4.2) (2024-02-08)


### Features

* **oauth2:** experimental support for oauth2 for api access ([e52624b](https://github.com/tomtom-international/maps-sdk-js/commit/e52624bdc9d5f5b5692a1ed80604b326cab2f6ad))

### [0.4.1](https://github.com/tomtom-international/maps-sdk-js/compare/v0.3.3...v0.4.1) (2024-02-02)

### [0.3.3](https://github.com/tomtom-international/maps-sdk-js/compare/v0.3.2...v0.3.3) (2024-01-29)


### Features

* **pois:** removing unused init param from pois module ([67947f8](https://github.com/tomtom-international/maps-sdk-js/commit/67947f89c4a7e4160a46ada4712f4d0713a65fa3))

### [0.3.2](https://github.com/tomtom-international/maps-sdk-js/compare/v0.3.1...v0.3.2) (2024-01-26)


### Features

* **sdk:** removing poi style part since in orbis pois are now part of the base map. Pins are still not supported, when/if they come it'll require another sdk update ([535376c](https://github.com/tomtom-international/maps-sdk-js/commit/535376c89d082b380f979596187933ca3132866b))

### [0.3.1](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.28...v0.3.1) (2024-01-17)


### Features

* Accept request params types to HasLngLat ([b11cc9f](https://github.com/tomtom-international/maps-sdk-js/commit/b11cc9fb2d657cdb2c32f549b2c5a579fd9b5089))
* Add Matrix Routing support ([c66a122](https://github.com/tomtom-international/maps-sdk-js/commit/c66a1229fba63325ee68d09094724b345a67f495))
* **ev:** new optional service parameter to filter out ev stations which have unknown availability ([8b68ce4](https://github.com/tomtom-international/maps-sdk-js/commit/8b68ce42f1d80d16943114613c83f17fcfcae401))
* **map:** adding more poi categories into map category groups ([38b83e9](https://github.com/tomtom-international/maps-sdk-js/commit/38b83e9b8358186c9346b117a30dd508311cc3b8))
* **map:** allowing to change visibility for selected layer groups in a base map module ([b0c4287](https://github.com/tomtom-international/maps-sdk-js/commit/b0c4287c7165d32d1e92f51c6bc9c719d119b6cc))
* **map:** changing how we add parameters to setStyle to allow them to grow better in the future ([638aa53](https://github.com/tomtom-international/maps-sdk-js/commit/638aa5361e43e54623b75e4fec9f21a15c29b4df))
* **map:** first ability to programmatically put and clean event states ([114985d](https://github.com/tomtom-international/maps-sdk-js/commit/114985dd545fe89c7f7267cf6170ff103c01e40f))
* **map:** improvements displaying charging stations along route ([d26f4a7](https://github.com/tomtom-international/maps-sdk-js/commit/d26f4a760db3b6ee1ae0c6963a6ca759d2892242))
* **map:** switching map display to Orbis ([f0ccef3](https://github.com/tomtom-international/maps-sdk-js/commit/f0ccef3009572e5dd821019ce8d748b94829567b))
* **places:** improving place by id response and opening hours properties, and dep upgrades ([2babd19](https://github.com/tomtom-international/maps-sdk-js/commit/2babd1994f9725f172bcdad8e9d5eb1c11cbdd8f))
* **places:** typing poicategory in place responses. ([fd792bf](https://github.com/tomtom-international/maps-sdk-js/commit/fd792bfe8efb0cb12fa718520256d4420beb79e7))
* **routing:** adding path point index for each instruction when parsing a route ([c448689](https://github.com/tomtom-international/maps-sdk-js/commit/c4486895878b33068558a4d8127ce5979231ab03))
* **routing:** cleaning up ldevr post parameters from sdk which should be better integrated in a future, to prevent examples with overlapping or confusing inputs. Updating map integration tests data file locations. ([86af87f](https://github.com/tomtom-international/maps-sdk-js/commit/86af87f942588b3d0d26ed0d60c2e1598482bb85))
* **routing:** displaying guidance instructions along route line ([5ff152f](https://github.com/tomtom-international/maps-sdk-js/commit/5ff152f72bb87b18145a940adf3e6189a0393892))
* **routing:** improving routing module tests ([fd766ce](https://github.com/tomtom-international/maps-sdk-js/commit/fd766ce2ced7e190bbeecddabcf5f1e8374680bc))
* **routing:** making some guidance and api version parameters optional, adding ldver and guidance routing module tests ([8ec0d1d](https://github.com/tomtom-international/maps-sdk-js/commit/8ec0d1d30571ac5dcd725fb6cd8b06fa4d3f5ae8))


### Bug Fixes

* **charging_stations:** adding title and adding features for alternatives ([bd3d941](https://github.com/tomtom-international/maps-sdk-js/commit/bd3d9416d831e731996da47e8dd03454cc91de4d))
* **charging_stations:** work in progress ([a782cf3](https://github.com/tomtom-international/maps-sdk-js/commit/a782cf330dc4dc16046aeef5d519427520235e61))
* **charging_stations:** work in progress ([306100b](https://github.com/tomtom-international/maps-sdk-js/commit/306100b868a9e8697f5b77eaabea47c085bba2f5))
* **EDXCE-89:** Adding TODO comments and updating a test ([d4cbff1](https://github.com/tomtom-international/maps-sdk-js/commit/d4cbff1cf9fb9c6d8ae3e04432d17dc54fda0f99))
* **EDXCE-89:** improvements ([15d4ff2](https://github.com/tomtom-international/maps-sdk-js/commit/15d4ff256820f3398bb6dd9cbd2be58394aff48f))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis improvements and some cleanup ([b8c59fe](https://github.com/tomtom-international/maps-sdk-js/commit/b8c59fe31234713184ff8216de77ed484163fb55))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis, work in progress, added ldEV ([4799acd](https://github.com/tomtom-international/maps-sdk-js/commit/4799acd65ba8cf6a0a7f59f00ab54e5d3ad790e6))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis, work in progress, fixing tests ([240d888](https://github.com/tomtom-international/maps-sdk-js/commit/240d8889f55a77505d8cda3d95a66ff7ae75aa83))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis, work in progress, lots of commented out code ([e260215](https://github.com/tomtom-international/maps-sdk-js/commit/e2602152809115674a7abaa26066fa71c27069f9))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis, work in progress, reverting back vehicleRestricted ([51159ae](https://github.com/tomtom-international/maps-sdk-js/commit/51159aed3b597bb71087d794ce49cf6bc79a8d6b))
* **EDXCE-89:** Moving Routing service to Orbis from Genesis, work in progress, support for new sections ([bd903dc](https://github.com/tomtom-international/maps-sdk-js/commit/bd903dc678dcedab524a9a8d374029e7445f02f3))
* **map:** fixing petrol station map poi mapping ([07b79a9](https://github.com/tomtom-international/maps-sdk-js/commit/07b79a9ca93be6ba959ede7f21813186ac87f181))
* **map:** hardening logic when styles and module changes are called quickly in succession ([68e6749](https://github.com/tomtom-international/maps-sdk-js/commit/68e67492a53feb9ee080f3f750a128769cd65781))
* **map:** hardening logic when styles and module changes are called quickly in succession ([aaa50bc](https://github.com/tomtom-international/maps-sdk-js/commit/aaa50bc3a9ce7d8c5bb207d038e6d3ca4e17338d))
* **map:** hardening logic when styles and module changes are called quickly in succession ([61bd4f5](https://github.com/tomtom-international/maps-sdk-js/commit/61bd4f5a68fc9dbff4a9c52d2972d4c1712f6732))
* **map:** multiple fixes related to changing style vs events and showing geojson items ([54b2bf6](https://github.com/tomtom-international/maps-sdk-js/commit/54b2bf6bf92523417d504d3001a31c349368a6f7))
* **map:** removing maxzoom for charging stations on route for now ([e9d8dc6](https://github.com/tomtom-international/maps-sdk-js/commit/e9d8dc69e576ab25add4a51721bc442f37f075c7))
* **map:** trying to allow for style data handler errors and still call the following ones. And minor misc fixes. ([aa69f65](https://github.com/tomtom-international/maps-sdk-js/commit/aa69f654a992470d4b07ebd7c4344f307bce6f08))
* **places:** attempting to mitigate strange glitch where showFeatures isn't updated in sdk geojson sources ([b9455a8](https://github.com/tomtom-international/maps-sdk-js/commit/b9455a8401b806e85ae0b71e79d69a5509d76155))
* rename project in veracode, blackduck scan workflows ([84d2559](https://github.com/tomtom-international/maps-sdk-js/commit/84d25598581a16a7bd3562e120f2d4a8cceaff50))
* **routing:** hardening the display of instruction lines on the map ([9a11f26](https://github.com/tomtom-international/maps-sdk-js/commit/9a11f2665e362c29743f9d8e872ef1c2610da3be))
* **routing:** trying to fix ts check in tests project ([766f8f1](https://github.com/tomtom-international/maps-sdk-js/commit/766f8f1fbb6d04350c27d9b266420fb4a15b9f10))
* **sdk:** flaky test mitigation ([7bace8f](https://github.com/tomtom-international/maps-sdk-js/commit/7bace8fdc842635964479d0b07656ac4923d8d9b))
* **sdk:** helping tests run better in ci cd ([b25c499](https://github.com/tomtom-international/maps-sdk-js/commit/b25c4999fb4ef596997b6199949aa69348c2b360))
* **sdk:** trying to make the geojson modules more resilient to quick changes in succession ([87b0937](https://github.com/tomtom-international/maps-sdk-js/commit/87b09377d74b8117b3da7f11074028452f3b8d83))
* **sdk:** trying to stabilize tests in ci cd ([48acb9b](https://github.com/tomtom-international/maps-sdk-js/commit/48acb9b3ce1bd59e4181b1003fd85c822c61727b))
* **tests:** enabling map mocked tests with local asset mocking ([ec13bbb](https://github.com/tomtom-international/maps-sdk-js/commit/ec13bbb0c856c8a613811ae33a7eece311f82e9f))
* **tests:** enabling puppeteer tests to still run while SDK imports trigger detection of document object due to embedded sdk assets ([6b6c395](https://github.com/tomtom-international/maps-sdk-js/commit/6b6c395dedcfd48ccfa2f527e6d5b7c815b1b445))

### [0.2.28](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.27...v0.2.28) (2023-11-07)


### Features

* **map:** increasing event delays trying to make tests pass with default ci/cd capacity ([03ba1ba](https://github.com/tomtom-international/maps-sdk-js/commit/03ba1baddc4a296c96c8fdf821b908b3f6d526e1))

### [0.2.27](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.26...v0.2.27) (2023-11-06)


### Features

* **map:** enhancing events to allow different handlers for the same source but different layers ([1b9b0c1](https://github.com/tomtom-international/maps-sdk-js/commit/1b9b0c100cf9e828383572ac60235ef868242d3d))

### [0.2.26](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.25...v0.2.26) (2023-11-01)


### Features

* **map:** allowing multiple base map modules with different layer groups ([dfb260d](https://github.com/tomtom-international/maps-sdk-js/commit/dfb260da1fdd9c9f9f3010a26e730a838766b7a5))

### [0.2.25](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.24...v0.2.25) (2023-10-27)


### Bug Fixes

* **core:** improvin polygon feature typing ([2ba8bf0](https://github.com/tomtom-international/maps-sdk-js/commit/2ba8bf0d777f5ce52be8db89c512cce9294ae966))

### [0.2.24](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.23...v0.2.24) (2023-10-27)


### Features

* **services:** reachable range service ([0a8e7b0](https://github.com/tomtom-international/maps-sdk-js/commit/0a8e7b0fb955b634803e3b4a4ea1d21315b3b89c))


### Bug Fixes

* change fail on severity to be only for the very high errors ([0bec42d](https://github.com/tomtom-international/maps-sdk-js/commit/0bec42da55e55d8482fbb300a3dce38e64a21fb1))

### [0.2.23](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.22...v0.2.23) (2023-10-05)


### Features

* **services:** adding optional callbacks for api requests and responses within each service ([8828c30](https://github.com/tomtom-international/maps-sdk-js/commit/8828c307a4b64cb855f984da6c86e8c15620a3b9))

### [0.2.22](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.21...v0.2.22) (2023-09-14)


### Bug Fixes

* **misc:** fixing tests and adding EV accessibility enum ([bd30175](https://github.com/tomtom-international/maps-sdk-js/commit/bd301755dfdbf9c469e0ac1a062fb3905b3de7e7))
* **misc:** removing incremental tsconfig setting to try to fix rollup bundling ([123dcba](https://github.com/tomtom-international/maps-sdk-js/commit/123dcbaa369caebb5b4cfb0718c37885a8642350))

### [0.2.21](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.20...v0.2.21) (2023-06-29)


### Features

* **hillshade:** applying final hillshade styles ([14f176f](https://github.com/tomtom-international/maps-sdk-js/commit/14f176f66b56c2789dcb0659fbab1616c860e240))

### [0.2.20](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.19...v0.2.20) (2023-06-14)


### Features

* **routing:** adding avoid options and sections ([73130e8](https://github.com/tomtom-international/maps-sdk-js/commit/73130e811e557ed7e5b7b4d97aeebac9a658a4c5))

### [0.2.19](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.18...v0.2.19) (2023-06-12)


### Bug Fixes

* **test:** trying to stabilize test ([97ca70d](https://github.com/tomtom-international/maps-sdk-js/commit/97ca70dfa8b10d985e69c27cafd4c4e1a14f740d))
* **test:** trying to stabilize test ([0b8741b](https://github.com/tomtom-international/maps-sdk-js/commit/0b8741b28019040e6ef25c8bf01a1dd9dd91487f))

### [0.2.18](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.17...v0.2.18) (2023-06-09)

### [0.2.17](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.16...v0.2.17) (2023-06-09)

### [0.2.16](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.15...v0.2.16) (2023-06-09)

### [0.2.15](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.14...v0.2.15) (2023-05-25)

### [0.2.14](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.13...v0.2.14) (2023-05-25)

### [0.2.13](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.12...v0.2.13) (2023-05-15)

### [0.2.12](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.11...v0.2.12) (2023-05-15)


### Features

* **places:** ability to conveniently show a single place or an array of them in addition to a feature collection ([956c27b](https://github.com/tomtom-international/maps-sdk-js/commit/956c27b1b4c5aa3abe47234791dc0feb3ab9c515))

### [0.2.11](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.10...v0.2.11) (2023-05-12)


### Bug Fixes

* **geometries:** small typing simplification / fix ([cd03a8a](https://github.com/tomtom-international/maps-sdk-js/commit/cd03a8a8c9f5f445079ed9b21c73df00016fc9ea))
* **map:** hotfix to allow supplying valid 0s as inputs ([d4b0a24](https://github.com/tomtom-international/maps-sdk-js/commit/d4b0a24583147f79096fc48c8f6b11e49d29e6d3))
* **services:** hot fixing unstable routing integration test ([0456f4f](https://github.com/tomtom-international/maps-sdk-js/commit/0456f4f19a3adfe15a38baf8a38ff94b36005cc6))

### [0.2.10](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.8...v0.2.10) (2023-05-11)


### Features

* **api reference:** add version labels to generated API reference ([c25fe0a](https://github.com/tomtom-international/maps-sdk-js/commit/c25fe0a37b58f0a3339a0076825d08d6662477f0))
* **guides:** update guide versions on release ([6eede6d](https://github.com/tomtom-international/maps-sdk-js/commit/6eede6dc911ebdf6ee0cf21ef52ff9b846ec034b))
* **map:** allowing to have multiple geometries modules for a map ([838c791](https://github.com/tomtom-international/maps-sdk-js/commit/838c79108c30590c65fb0e95719209ac54cc59f3))

### [0.2.8](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.7...v0.2.8) (2023-05-10)


### Features

* **map:** allowing to change the style while keeping the previous style parts auto-adjusted to the new style variant ([fb721b0](https://github.com/tomtom-international/maps-sdk-js/commit/fb721b062957ce85dedb70bea86aed9e057664f7))

### [0.2.7](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.3...v0.2.7) (2023-05-10)


### Features

* **855:** moving from excluding to including modules ([01a0b5c](https://github.com/tomtom-international/maps-sdk-js/commit/01a0b5c60a2ce69ce747b2c3e988b4515721ef7e))
* **855:** splitting traffic flow into its own module ([935eca0](https://github.com/tomtom-international/maps-sdk-js/commit/935eca02462f3b62577dddfd3c4e8cb2cce0d77c))
* **855:** splitting traffic flow into its own module ([5531dd5](https://github.com/tomtom-international/maps-sdk-js/commit/5531dd51ae15e8aa4cf93999d9ba3562fa8f8bf5))
* **855:** splitting traffic incidents into its own module ([07f0374](https://github.com/tomtom-international/maps-sdk-js/commit/07f0374c59b50db3f773f1d0e87733bf4c7e3a16))
* **855:** trying to auto update the modules if initiated ([fb128d5](https://github.com/tomtom-international/maps-sdk-js/commit/fb128d51d9325474f3538d22f458be27be9f7bd8))
* **855:** trying to auto update the modules if initiated ([404498b](https://github.com/tomtom-international/maps-sdk-js/commit/404498beaa87a6409c04cbc5b331035945b4d963))
* **855:** trying to auto update the modules if initiated ([a13bc7c](https://github.com/tomtom-international/maps-sdk-js/commit/a13bc7ce28eac3dae3ba08eab0eac6b61d7f592d))
* **api reference:** update to next version of typedoc and typedoc-plugin-markdown ([b9a889c](https://github.com/tomtom-international/maps-sdk-js/commit/b9a889cb04b9ab18d16fa375a48f94f96067ab8c))
* **api ref:** render classes to their own file with updated file output structure ([488ea5b](https://github.com/tomtom-international/maps-sdk-js/commit/488ea5b06a31fa5214d046bc42711e1ab1bfc083))
* **docs:** updates in project setup guide based on feedback ([d863053](https://github.com/tomtom-international/maps-sdk-js/commit/d86305340f3bcbe5a791c1accf47d98a69210623))
* **docs:** upgrading guides, focusing on service customization ([77e6148](https://github.com/tomtom-international/maps-sdk-js/commit/77e61485de4fd063761c60ab2b32ddcbf2719757))
* **docs:** upgrading guides, focusing on services ([ec52e51](https://github.com/tomtom-international/maps-sdk-js/commit/ec52e51ad26d7d9cfab03781d165ee44c03b9a5c))
* **docs:** upgrading guides, focusing on traffic module ([c7be10f](https://github.com/tomtom-international/maps-sdk-js/commit/c7be10f70bf565885ca7790dae11fc8f2466c42f))
* **map:** fixing ability to auto-add multiple style modules close together, including new traffic flow module in tests, and some renamings ([1610beb](https://github.com/tomtom-international/maps-sdk-js/commit/1610beb995260c119a875d9f44466f9df5d8645e))
* **map:** making the ability to auto reload style on module init optional, false by default, adding tests, adding support for pois module auto-addition to style, refactorings ([3cd7b9b](https://github.com/tomtom-international/maps-sdk-js/commit/3cd7b9b80f05ab7c9fdcb0f1c2165911650eec1b))

### [0.2.6](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.3...v0.2.6) (2023-05-09)


### Features

* **docs:** upgrading guides, focusing on service customization ([77e6148](https://github.com/tomtom-international/maps-sdk-js/commit/77e61485de4fd063761c60ab2b32ddcbf2719757))
* **docs:** upgrading guides, focusing on services ([ec52e51](https://github.com/tomtom-international/maps-sdk-js/commit/ec52e51ad26d7d9cfab03781d165ee44c03b9a5c))
* **docs:** upgrading guides, focusing on traffic module ([c7be10f](https://github.com/tomtom-international/maps-sdk-js/commit/c7be10f70bf565885ca7790dae11fc8f2466c42f))

### [0.2.5](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.3...v0.2.5) (2023-05-08)


### Features

* **docs:** upgrading guides, focusing on service customization ([77e6148](https://github.com/tomtom-international/maps-sdk-js/commit/77e61485de4fd063761c60ab2b32ddcbf2719757))
* **docs:** upgrading guides, focusing on services ([ec52e51](https://github.com/tomtom-international/maps-sdk-js/commit/ec52e51ad26d7d9cfab03781d165ee44c03b9a5c))
* **docs:** upgrading guides, focusing on traffic module ([c7be10f](https://github.com/tomtom-international/maps-sdk-js/commit/c7be10f70bf565885ca7790dae11fc8f2466c42f))

### [0.2.4](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.3...v0.2.4) (2023-05-03)


### Features

* **docs:** upgrading guides, focusing on service customization ([77e6148](https://github.com/tomtom-international/maps-sdk-js/commit/77e61485de4fd063761c60ab2b32ddcbf2719757))
* **docs:** upgrading guides, focusing on services ([ec52e51](https://github.com/tomtom-international/maps-sdk-js/commit/ec52e51ad26d7d9cfab03781d165ee44c03b9a5c))
* **docs:** upgrading guides, focusing on traffic module ([c7be10f](https://github.com/tomtom-international/maps-sdk-js/commit/c7be10f70bf565885ca7790dae11fc8f2466c42f))

### [0.2.3](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.2...v0.2.3) (2023-04-26)


### Features

* **map:** when custom style urls come with an api key, we use it, regardless of the configured one via sdk ([1c7fde5](https://github.com/tomtom-international/maps-sdk-js/commit/1c7fde5a4af058d8e08c1f8c9e55efc092a360dc))

### [0.2.2](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.1...v0.2.2) (2023-04-26)


### Bug Fixes

* **EV:** resolve issue related to search result parsing ([b9e6ede](https://github.com/tomtom-international/maps-sdk-js/commit/b9e6ede5e3f798292c8b00f992d98a069ceb8868))

### [0.2.1](https://github.com/tomtom-international/maps-sdk-js/compare/v0.2.0...v0.2.1) (2023-04-25)


### Features

* **baseMap:** Add BaseMap module ([9e09390](https://github.com/tomtom-international/maps-sdk-js/commit/9e093907abf7d3f4029485c42ae6f17a88355f06))
* **basemap:** Update click logic and add test(failing) ([772693c](https://github.com/tomtom-international/maps-sdk-js/commit/772693c720772ead377b7233e0060ae5e545e18f))
* **docs:** updating guides, focusing on user events and places ([f1d7a03](https://github.com/tomtom-international/maps-sdk-js/commit/f1d7a03aeba03bda5b9f4aa37f58423ace01754b))
* **map:** ensuring map module constructors are private ([d419522](https://github.com/tomtom-international/maps-sdk-js/commit/d41952241a44717b6d69b0a330e43a7e346d58d9))
* **map:** making base map constructor private ([defbec4](https://github.com/tomtom-international/maps-sdk-js/commit/defbec4f1dd3c19792ffe176ad577827f3c281cc))
* **map:** simplifying/fixing base map init ([c670933](https://github.com/tomtom-international/maps-sdk-js/commit/c67093349578a8cdb63c86647d978ab283918bc0))


### Bug Fixes

* **basemap:** Move initBasemap to TestUtils, fix merging issue. ([c837f09](https://github.com/tomtom-international/maps-sdk-js/commit/c837f098d8b5735d30717f0499bc9f9f1120bc15))

## [0.2.0](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.112...v0.2.0) (2023-04-20)


### ⚠ BREAKING CHANGES

* **map:** map module names are changed

### Features

* **documentation:** updating and adding guides ([ecb61a5](https://github.com/tomtom-international/maps-sdk-js/commit/ecb61a5f4d4b19b2ad60a91ac5de3fce8381667e))
* **map:** renaming map modules ([edc0b77](https://github.com/tomtom-international/maps-sdk-js/commit/edc0b77e3d85299e62565bea409b277f28915009))

### [0.1.112](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.111...v0.1.112) (2023-04-14)


### Features

* **511:** first iteration of route customization ([d3d43c7](https://github.com/tomtom-international/maps-sdk-js/commit/d3d43c73ed4e152725ee27754d70dcf533c652dd))
* **511:** more updates to the logic and tests ([6e94b7d](https://github.com/tomtom-international/maps-sdk-js/commit/6e94b7d5b7b1e8b4e26f792fc6190b69af026704))
* **511:** more updates to the logic and tests ([3564961](https://github.com/tomtom-international/maps-sdk-js/commit/3564961797969ccc16d385feb00d085e3ce48125))


### Bug Fixes

* **routing:** improving test stability ([8ad239f](https://github.com/tomtom-international/maps-sdk-js/commit/8ad239f7eb41f6c79f7c00a16132f048f2d41412))

### [0.1.111](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.110...v0.1.111) (2023-04-13)


### Bug Fixes

* **geometry:** removing straight labels config, centralizing key map style layer ids, reusing from routing module ([5c96386](https://github.com/tomtom-international/maps-sdk-js/commit/5c96386a7ace457cbc6cf4c8fafb7ed3e01569e8))

### [0.1.110](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.109...v0.1.110) (2023-04-12)


### Features

* **map:** adding monoDark style ([93006e7](https://github.com/tomtom-international/maps-sdk-js/commit/93006e78ff313a8c523b1850b240bd23df8d5275))

### [0.1.109](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.108...v0.1.109) (2023-04-07)

### [0.1.108](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.107...v0.1.108) (2023-04-07)


### Features

* **docs:** adding dev portal links to main bundles ([2b2a189](https://github.com/tomtom-international/maps-sdk-js/commit/2b2a189122db88cde8efbfb969a575d8d15847bf))
* **geometry:**  Remove unused import ([0ef8fce](https://github.com/tomtom-international/maps-sdk-js/commit/0ef8fcee322ee3d333834cc707530d436d29b19a))
* **geometry:**  Restore configuration ([310e881](https://github.com/tomtom-international/maps-sdk-js/commit/310e88135c7792397f3af225f7d406d4f77fdf20))

### [0.1.107](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.106...v0.1.107) (2023-04-06)


### Features

* **docs:** adding imagery to readme and improving format ([06be075](https://github.com/tomtom-international/maps-sdk-js/commit/06be0756ed241a5cda58444ca5739e5afa81fcc1))
* **docs:** adding link to TT dev portal ([f6d75ee](https://github.com/tomtom-international/maps-sdk-js/commit/f6d75ee48a4ef885fa283cfd222dc2496f56bd70))
* **docs:** adding link to TT logo ([104ef27](https://github.com/tomtom-international/maps-sdk-js/commit/104ef276dceb73916721d7fc82f1ed1a790c58da))
* **docs:** improving readme files and minor unrelated cleanups ([a26dac3](https://github.com/tomtom-international/maps-sdk-js/commit/a26dac352b2994dc64df35aa4cfc269559fdabfb))

### [0.1.106](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.105...v0.1.106) (2023-04-04)


### Features

* **geometry:**  Accept layer options typed ([0c5eeb0](https://github.com/tomtom-international/maps-sdk-js/commit/0c5eeb074b2d87618b60b02bbb196f1c94ff7971))
* **geometry:**  Apply layer positioning ([9ff55f4](https://github.com/tomtom-international/maps-sdk-js/commit/9ff55f43e7b1ee3069f27eba4d4e14a0096c69c8))
* **geometry:**  Update geometry position configuration ([a8fc302](https://github.com/tomtom-international/maps-sdk-js/commit/a8fc3020ec22264023b68c6cbcde4caf0a5e4540))

### [0.1.105](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.104...v0.1.105) (2023-04-03)


### Features

* **language:** restoring map language after changing map style ([75a7f6a](https://github.com/tomtom-international/maps-sdk-js/commit/75a7f6ad1b7ab6e16bac6d987d65f16b363b5d07))


### Bug Fixes

* **multiple-geometries:** Removing unused import ([b9fec7d](https://github.com/tomtom-international/maps-sdk-js/commit/b9fec7dbf1fca786df9c879885eda1df0856a269))
* **multiple-geometries:** update configurations to one line. ([c7ed358](https://github.com/tomtom-international/maps-sdk-js/commit/c7ed3586e4d1efdbdd3f6aa388186899bd512d50))

### [0.1.104](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.103...v0.1.104) (2023-03-30)


### Bug Fixes

* **language:** hotfix to set map language properly from global config ([af51938](https://github.com/tomtom-international/maps-sdk-js/commit/af51938cc3b4f78acb556de65ae53681ee9c1594))

### [0.1.103](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.102...v0.1.103) (2023-03-30)

### [0.1.102](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.101...v0.1.102) (2023-03-30)


### Features

* **map:** restore modules state when changing map style ([8fd79ec](https://github.com/tomtom-international/maps-sdk-js/commit/8fd79ecc33961c1b9fba665338dfe50011bb14b7))
* **state:** restoring state for map modules, rebasing on main ([9f20484](https://github.com/tomtom-international/maps-sdk-js/commit/9f204849127aeac500f7e5b17fccd329df5fd5ac))

### [0.1.101](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.100...v0.1.101) (2023-03-30)


### Bug Fixes

* removing unused imports/typo ([f52be6d](https://github.com/tomtom-international/maps-sdk-js/commit/f52be6d8d3cbd9f13052ddf1ef8c8ecf784ffe0e))

### [0.1.100](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.99...v0.1.100) (2023-03-27)


### Bug Fixes

* **core:** Geometries default props ([904f62d](https://github.com/tomtom-international/maps-sdk-js/commit/904f62dff50a16ea97773dad1e945d65121c4b9f))

### [0.1.99](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.98...v0.1.99) (2023-03-27)


### Features

* **events:** removing paddingBoxUpdateOnZoom property in favor of paddingBox and existing improved behaviour for event target precision ([c55ceee](https://github.com/tomtom-international/maps-sdk-js/commit/c55ceee385dadb098ab5be0079038379114b4d2b))

### [0.1.98](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.97...v0.1.98) (2023-03-27)

### [0.1.97](https://github.com/tomtom-international/maps-sdk-js/compare/v0.1.96...v0.1.97) (2023-03-23)


### Features

* **events:** setting hover/click event props and default styling for places ([22e7330](https://github.com/tomtom-international/maps-sdk-js/commit/22e7330690b7697fbb1ca66bef3039b0db03cbc5))

### [0.1.96](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.95...v0.1.96) (2023-03-20)


### Features

* **743:** exposing sources and layers from modules ([a8099e2](https://github.com/tomtom-international/go-sdk-js/commit/a8099e238826111f65a00a4a2f871c1e12ceff0e))
* **743:** refactoring to only expose ids ([446437f](https://github.com/tomtom-international/go-sdk-js/commit/446437f1a11eba225f363c3f3c0102a557e8a315))

### [0.1.95](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.94...v0.1.95) (2023-03-15)


### Features

* **services:** add summary properties to search ([f83ec26](https://github.com/tomtom-international/go-sdk-js/commit/f83ec26a31f6fa036a19a6872f5e628bdc7746ae))

### [0.1.94](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.93...v0.1.94) (2023-03-13)


### Bug Fixes

* id isn't required in POIs ([6646de0](https://github.com/tomtom-international/go-sdk-js/commit/6646de02c83b7233952380633e954dc1f71df23a))
* **service:** update generateId function ([d3484cd](https://github.com/tomtom-international/go-sdk-js/commit/d3484cd1475830713320cbffc01a0b44d439f96f))

### [0.1.93](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.92...v0.1.93) (2023-03-08)


### Features

* **ev:** fixing search ev poi parsing and adding non-real time connector counts ([7c3f01c](https://github.com/tomtom-international/go-sdk-js/commit/7c3f01cb98167262515059d58ccee4db2c6cfcc2))

### [0.1.92](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.90...v0.1.92) (2023-03-08)


### Features

* **sdk:** upgrading version ([2c519b7](https://github.com/tomtom-international/go-sdk-js/commit/2c519b77d7a992a846d39d743d6bcdc490e74abe))

### [0.1.89](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.87...v0.1.89) (2023-03-02)

### [0.1.86](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.85...v0.1.86) (2023-03-01)


### Features

* allow customizing places text ([6dcb600](https://github.com/tomtom-international/go-sdk-js/commit/6dcb600fefbe5b7b307b283ff6efc283506573bf))
* blackduck setup ([bc43727](https://github.com/tomtom-international/go-sdk-js/commit/bc43727acb6f9d1a102a2faf05a18b75682c7a47))
* **routing:** ldevr support ([6c65e27](https://github.com/tomtom-international/go-sdk-js/commit/6c65e271c71a2d85aff1ea7d60913f1c702c92bf))


### Bug Fixes

* **map:** Fix code smells ([b7ad8de](https://github.com/tomtom-international/go-sdk-js/commit/b7ad8debf37f24a98d11ff7027ee8078ee9ea4fc))
* use config parameter in places module methods instead of this.config ([94c467a](https://github.com/tomtom-international/go-sdk-js/commit/94c467a8c5f67f4f194099a4914f98da6e9ba35a))

### [0.1.85](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.84...v0.1.85) (2023-02-20)

### [0.1.84](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.83...v0.1.84) (2023-02-17)


### Features

* **traffic:** ensuring traffic module config stays up to date ([f3bf830](https://github.com/tomtom-international/go-sdk-js/commit/f3bf830e8525dd31adc1d63b443a9ebe629fe02f))


### Bug Fixes

* **places:** fixing how a layer paint and layout props are changed ([ff6c657](https://github.com/tomtom-international/go-sdk-js/commit/ff6c6575edd69e5b8817a399b086c02a084402af))
* sandbox run condition ([f736aeb](https://github.com/tomtom-international/go-sdk-js/commit/f736aeb52a095dac173c577290d61ca0b030944d))

### [0.1.83](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.82...v0.1.83) (2023-02-15)

### [0.1.82](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.81...v0.1.82) (2023-02-13)


### Bug Fixes

* **traffic:** fixing icon vs default visibility logic, renaming methods, adding tests ([390e351](https://github.com/tomtom-international/go-sdk-js/commit/390e351ae35bb33f60ba4a4f54c54469ec1cfb57))

### [0.1.81](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.80...v0.1.81) (2023-02-11)


### Bug Fixes

* **traffic:** fixing flow filtering bug ([3c68645](https://github.com/tomtom-international/go-sdk-js/commit/3c686452115069a4648eb8438bf890ee9d35c0cd))

### [0.1.80](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.79...v0.1.80) (2023-02-09)


### Features

* **589:** adding formatting utility functions ([3b1a3dd](https://github.com/tomtom-international/go-sdk-js/commit/3b1a3dd0db9c04aef775f5d8de6ee6854fdf512f))
* **589:** naming refactoring ([48ff932](https://github.com/tomtom-international/go-sdk-js/commit/48ff932e427a5d2e9875f85fafec87389f8f9376))
* custom place layer icons ([de95b85](https://github.com/tomtom-international/go-sdk-js/commit/de95b850a6f52a9e6eb97b5a19c5747d126a5f31))
* **map:** allowing direct traffic filter changes, adding tests and misc improvements ([4a673d2](https://github.com/tomtom-international/go-sdk-js/commit/4a673d20f91520d77e973cf18ea9cd1388b34d09))
* **map:** improving how configs are applied ([14ee8d8](https://github.com/tomtom-international/go-sdk-js/commit/14ee8d8533145d8ba05c74dda2458132cd50cae9))


### Bug Fixes

* Update import ([f161f60](https://github.com/tomtom-international/go-sdk-js/commit/f161f6096befc9636fdca065356b7db10d24cfc0))

### [0.1.79](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.78...v0.1.79) (2023-02-02)


### Bug Fixes

* **#660:** make language param optional in auto-complete search ([8e368b2](https://github.com/tomtom-international/go-sdk-js/commit/8e368b2a33e473db8749603f85a05a5596935537)), closes [#660](https://github.com/tomtom-international/go-sdk-js/issues/660)
* **#660:** make language param optional in auto-complete search ([57f036b](https://github.com/tomtom-international/go-sdk-js/commit/57f036bbd1a404c8eaa718386cb8c70ce6813900)), closes [#660](https://github.com/tomtom-international/go-sdk-js/issues/660)

### [0.1.78](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.77...v0.1.78) (2023-02-02)


### Features

* **traffic:** filtering options for vector tiles traffic ([a6867e7](https://github.com/tomtom-international/go-sdk-js/commit/a6867e7fc4954e201213b8bcb230a561811336e2))


### Bug Fixes

* **520:** making sure all sections use endIndex inclusively ([bc654cf](https://github.com/tomtom-international/go-sdk-js/commit/bc654cf25b91f91ce55f195606829163386636eb))
* **520:** making sure all sections use endIndex inclusively ([aad40ca](https://github.com/tomtom-international/go-sdk-js/commit/aad40caf4a6f6e5d01998dde06aec9fbff2e9333))
* veracode sandboxname parameter to add release scans out of sandbox ([1deecc8](https://github.com/tomtom-international/go-sdk-js/commit/1deecc81b19ebd072fb693dd50ca00202fb89f35))

### [0.1.77](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.76...v0.1.77) (2023-01-30)


### Features

* allow filtering pois to include or exclude categories ([22adc8a](https://github.com/tomtom-international/go-sdk-js/commit/22adc8abde6ecf99689aa43b9bb7a31744e4ff34))
* consider existing poi layer filters ([6b7e8d0](https://github.com/tomtom-international/go-sdk-js/commit/6b7e8d08ba69371daea5c36d24bfc1012ab975c9))


### Bug Fixes

* use proper expression type ([6ecb984](https://github.com/tomtom-international/go-sdk-js/commit/6ecb984cf759bb0379b1fb609caab47b85e4ceb2))
* wait until previous features count change ([2a87bf6](https://github.com/tomtom-international/go-sdk-js/commit/2a87bf62165258056458f1571580708b4083255c))

### [0.1.76](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.75...v0.1.76) (2023-01-24)

### [0.1.75](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.74...v0.1.75) (2023-01-23)


### Bug Fixes

* **maps:** Fix typo ([9ebed6f](https://github.com/tomtom-international/go-sdk-js/commit/9ebed6f96e06df8bbbed5178f90f69ba4e101a0c))

### [0.1.74](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.73...v0.1.74) (2023-01-19)


### Features

* **routing:** route reconstruction second iteration ([3b02b3b](https://github.com/tomtom-international/go-sdk-js/commit/3b02b3b8ab81541c8badc530d1bd55ad1ba41be2))


### Bug Fixes

* **#604:** exclude poi from base map when configured as part of Map style ([62e1777](https://github.com/tomtom-international/go-sdk-js/commit/62e1777dc8717a86f5424183c82de328fc0392a6)), closes [#604](https://github.com/tomtom-international/go-sdk-js/issues/604)
* add timestamp to veracode version to avoid duplicates ([917a5ca](https://github.com/tomtom-international/go-sdk-js/commit/917a5ca5938f51a5e7282dfc21f66cd4942ff0a3))

### [0.1.73](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.70...v0.1.73) (2023-01-16)


### Features

* notify slack only on veracode scan failure ([932abed](https://github.com/tomtom-international/go-sdk-js/commit/932abed3692d6d474711d7367357f001ebda960d))
* notify slack with veracode scan result ([1711f90](https://github.com/tomtom-international/go-sdk-js/commit/1711f90d8ec4f8461ae0a9ce88d5327cce643e25))
* **routing:** exporting new function for layers context plus minor non-functional improvements ([638b6aa](https://github.com/tomtom-international/go-sdk-js/commit/638b6aac562482d7afae7ef68115d68aa2ef73df))
* **routing:** route reconstruction ([50c5cb6](https://github.com/tomtom-international/go-sdk-js/commit/50c5cb6bd11e4426d3803e16bf5248c5dd19fd14))
* run main scans in a sandbox, move pull request  scans to pipeline ([12a7071](https://github.com/tomtom-international/go-sdk-js/commit/12a707141a51b1dd4a048663ba368ff0bbc10451))
* run veracode pipeline scan for push ([5492c64](https://github.com/tomtom-international/go-sdk-js/commit/5492c64d34cf11e5d02ee7130ec3f18b7b60a65b))

### [0.1.72](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.70...v0.1.72) (2023-01-16)


### Features

* notify slack only on veracode scan failure ([932abed](https://github.com/tomtom-international/go-sdk-js/commit/932abed3692d6d474711d7367357f001ebda960d))
* notify slack with veracode scan result ([1711f90](https://github.com/tomtom-international/go-sdk-js/commit/1711f90d8ec4f8461ae0a9ce88d5327cce643e25))
* **routing:** exporting new function for layers context plus minor non-functional improvements ([638b6aa](https://github.com/tomtom-international/go-sdk-js/commit/638b6aac562482d7afae7ef68115d68aa2ef73df))
* **routing:** route reconstruction ([50c5cb6](https://github.com/tomtom-international/go-sdk-js/commit/50c5cb6bd11e4426d3803e16bf5248c5dd19fd14))
* run main scans in a sandbox, move pull request  scans to pipeline ([12a7071](https://github.com/tomtom-international/go-sdk-js/commit/12a707141a51b1dd4a048663ba368ff0bbc10451))
* run veracode pipeline scan for push ([5492c64](https://github.com/tomtom-international/go-sdk-js/commit/5492c64d34cf11e5d02ee7130ec3f18b7b60a65b))

### [0.1.71](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.70...v0.1.71) (2023-01-15)

### [0.1.70](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.69...v0.1.70) (2023-01-11)


### Bug Fixes

* **map:** Set EventsProxy enable method to public ([af25af2](https://github.com/tomtom-international/go-sdk-js/commit/af25af289cc57b307479131a37a664918d4d306d))

### [0.1.69](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.68...v0.1.69) (2023-01-10)


### Features

* add autocomplete service ([3b76260](https://github.com/tomtom-international/go-sdk-js/commit/3b762605fe4bf00ec5ff4aec0e29bf0462c29a10))

### [0.1.68](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.67...v0.1.68) (2023-01-04)


### Bug Fixes

* **543:** possible solution ([c2b4b5b](https://github.com/tomtom-international/go-sdk-js/commit/c2b4b5bd67ebe89a45559d94709e673a951eb787))

### [0.1.67](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.66...v0.1.67) (2022-12-27)


### Features

* **routing:** changing routing response type into a geojson extension ([6f74f99](https://github.com/tomtom-international/go-sdk-js/commit/6f74f99e4c3b4a18599f9b8f0b1c4f2300ed7b97))

### [0.1.66](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.65...v0.1.66) (2022-12-20)


### Features

* **map:** allowing to change route selection ([7d51241](https://github.com/tomtom-international/go-sdk-js/commit/7d51241e432c9b66161af3c8bda6d816d9f4c6f4))


### Bug Fixes

* geojson object validation schema ([2bcc054](https://github.com/tomtom-international/go-sdk-js/commit/2bcc054895b2ea10726544fc210176207f34a449))

### [0.1.65](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.64...v0.1.65) (2022-12-15)


### Features

* add gitBounds method to sdk map ([61bc455](https://github.com/tomtom-international/go-sdk-js/commit/61bc45524479aa7e16e41b8e15eaca812c6776e1))


### Bug Fixes

* **search-schema-validation:** allow number|POICategory as part of search poi category ([c2a8c67](https://github.com/tomtom-international/go-sdk-js/commit/c2a8c67f7d7978d5efe01880b1b6ca4a85f47569))
* **tests:** improving integration test stability ([a82d668](https://github.com/tomtom-international/go-sdk-js/commit/a82d668a576eb39daa8580e928dbcf216b1092dc))

### [0.1.64](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.63...v0.1.64) (2022-12-15)


### Features

* **map:** adding support for alternatives by default ([05013a4](https://github.com/tomtom-international/go-sdk-js/commit/05013a4262ea8fde205df781ed9f0fa56dcc63c3))

### [0.1.63](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.62...v0.1.63) (2022-12-14)


### Features

* integrate with fuzzy search service and expose general search call ([0dcb0c8](https://github.com/tomtom-international/go-sdk-js/commit/0dcb0c877aafbf35f01ec317b36ced7d16f88f3e))
* **map:** adding more route sections on the map ([7b3e40d](https://github.com/tomtom-international/go-sdk-js/commit/7b3e40d12b786a17a6507a2199c5ebb76e508c2f))
* **map:** adding vehicle restricted sections, refactored from travel mode "other" sections ([99d19b3](https://github.com/tomtom-international/go-sdk-js/commit/99d19b376bf1f627336960c1eb0e6db228a7d1d5))

### [0.1.62](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.61...v0.1.62) (2022-12-06)

### [0.1.61](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.60...v0.1.61) (2022-12-06)

### [0.1.60](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.59...v0.1.60) (2022-12-01)


### Features

* **map:** adding support for route sections on the map ([a557a6c](https://github.com/tomtom-international/go-sdk-js/commit/a557a6cbd821910c3b35c9ee7ad1de9bf20fab35))


### Bug Fixes

* [#539](https://github.com/tomtom-international/go-sdk-js/issues/539): fix hasLnglatSchema ([a53030d](https://github.com/tomtom-international/go-sdk-js/commit/a53030dc832cc62b1915630f8ab24456177a175a))
* [#539](https://github.com/tomtom-international/go-sdk-js/issues/539): fix/add tests containing hasLnglatSchema ([d5ccf0d](https://github.com/tomtom-international/go-sdk-js/commit/d5ccf0d730b7ec461da9b77e553dece4159e3875))

### [0.1.59](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.58...v0.1.59) (2022-11-29)


### Bug Fixes

* **routing:** add carpool section type ([26af7ae](https://github.com/tomtom-international/go-sdk-js/commit/26af7ae7dc475801386985e207ec9021fb87edf6))

### [0.1.58](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.57...v0.1.58) (2022-11-28)


### Features

* **map:** [#70](https://github.com/tomtom-international/go-sdk-js/issues/70) first iteration to render routes on the map ([9e7a1f3](https://github.com/tomtom-international/go-sdk-js/commit/9e7a1f3485e8a178a7104324aafbe4c4f8be85c6))

### [0.1.57](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.56...v0.1.57) (2022-11-24)


### Bug Fixes

* **map:** Move check config statement outside of RuntimeSource check ([d2c8a82](https://github.com/tomtom-international/go-sdk-js/commit/d2c8a82e118fd2c7a2fd714d87e7d48427aa1eaf))

### [0.1.56](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.55...v0.1.56) (2022-11-24)


### Bug Fixes

* **496:** removing invert from SDK, moving it to examples ([0fe1b6c](https://github.com/tomtom-international/go-sdk-js/commit/0fe1b6ca0c549cfd20d70510002d4c2ae0102d96))
* increasing reference kpi for response parser performance tests ([fd85f2b](https://github.com/tomtom-international/go-sdk-js/commit/fd85f2b271dfea16ee6813fb81d8ed5a3eaf7fb7))
* increasing reference kpi for response parser performance tests ([564677c](https://github.com/tomtom-international/go-sdk-js/commit/564677c9b716e59f96c2fc31eb7179f2c25cad65))
* increasing reference kpi for response parser performance tests ([8f40437](https://github.com/tomtom-international/go-sdk-js/commit/8f40437300cd95f12b9d6a5b880e2e8e00c17b52))
* increasing reference kpi for response parser performance tests ([5ed7400](https://github.com/tomtom-international/go-sdk-js/commit/5ed7400d68eeba5e096510463c7a7e4cffd65adc))

### [0.1.55](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.54...v0.1.55) (2022-11-17)


### Features

* embed poi categories in the sdk ([4a14ba3](https://github.com/tomtom-international/go-sdk-js/commit/4a14ba3c9d884b7408946d7f3b2f33cd23942a81))

### [0.1.54](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.52...v0.1.54) (2022-11-16)


### Features

* adding missing customize obj for services, export place by id service ([d8d35fc](https://github.com/tomtom-international/go-sdk-js/commit/d8d35fc43c73923ebf19859e1854319a87e80579))
* **map:** geometry data related improvements, added tests and refactoring ([ea699df](https://github.com/tomtom-international/go-sdk-js/commit/ea699dfc8a318b7972066b1e777ac0524c556401))

### [0.1.53](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.52...v0.1.53) (2022-11-16)


### Features

* adding missing customize obj for services, export place by id service ([d8d35fc](https://github.com/tomtom-international/go-sdk-js/commit/d8d35fc43c73923ebf19859e1854319a87e80579))
* **map:** geometry data related improvements, added tests and refactoring ([ea699df](https://github.com/tomtom-international/go-sdk-js/commit/ea699dfc8a318b7972066b1e777ac0524c556401))

### [0.1.52](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.51...v0.1.52) (2022-11-14)


### Features

* create mapLanguage module ([1d45823](https://github.com/tomtom-international/go-sdk-js/commit/1d458237c3bd1b326bf70540203bbcd6e12c7932))
* **search:** adding more supported geometries search input types for convenience ([0c51ffb](https://github.com/tomtom-international/go-sdk-js/commit/0c51ffba0a88ea2d4324f7afce4efe17e4227eb6))
* **search:** adding more supported geometry data service input types for convenience ([8c9bc41](https://github.com/tomtom-international/go-sdk-js/commit/8c9bc41169cdc014741cc5a36e40227f8dcc0bfe))


### Bug Fixes

* make sure map is ready before setting language ([9658c84](https://github.com/tomtom-international/go-sdk-js/commit/9658c841308fb9cd54283b163138fde90018e95b))

### [0.1.51](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.50...v0.1.51) (2022-11-11)


### Bug Fixes

* **map:** workaround against maplibre style loading state stability issues ([1c023bd](https://github.com/tomtom-international/go-sdk-js/commit/1c023bd46fc5bc13c9afd86ce94a96455f6e342c))

### [0.1.50](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.49...v0.1.50) (2022-11-10)


### Bug Fixes

* **tests:** test stability fixes ([791c841](https://github.com/tomtom-international/go-sdk-js/commit/791c841537e2dc721f6ca6e62829e329114af7b0))

### [0.1.49](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.48...v0.1.49) (2022-11-10)


### Bug Fixes

* **map:** ensuring the map is ready when calling map module methods ([4e2c4b3](https://github.com/tomtom-international/go-sdk-js/commit/4e2c4b344019f1d9abea06cd24e4fbd5c4f6f0cf))
* **services:** exporting geometry data response and adding it to service signature ([73779b2](https://github.com/tomtom-international/go-sdk-js/commit/73779b2caac2045045f7790e2913287b19b10d64))
* **tests:** test stability fixes ([402b90e](https://github.com/tomtom-international/go-sdk-js/commit/402b90ec59e40b367aaa8c32f7043f7d2083333f))

### [0.1.48](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.47...v0.1.48) (2022-11-09)


### Bug Fixes

* **services:** Fixing geometry search inputs and geometry data export ([7564d34](https://github.com/tomtom-international/go-sdk-js/commit/7564d34720e4bb838309f1401541bdacdbb80397))

### [0.1.47](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.46...v0.1.47) (2022-11-04)


### Features

* add charging availability service ([efbd5bf](https://github.com/tomtom-international/go-sdk-js/commit/efbd5bf410aa0f582d657e5d79430f8770aa8c65))
* add localize map method ([42c1984](https://github.com/tomtom-international/go-sdk-js/commit/42c19842020e4e3ff388af9333e72a04b30ec893))
* init map with locale coming from goSDK config ([69b21ea](https://github.com/tomtom-international/go-sdk-js/commit/69b21ead63f9115946901aff763de37c6d116db7))
* **map:** Map GeoJSON places ([e11e5c5](https://github.com/tomtom-international/go-sdk-js/commit/e11e5c5629d2c3508e10a3aa0d826f2154273618))


### Bug Fixes

* check if rtl plugin was loaded/deferred ([10f3412](https://github.com/tomtom-international/go-sdk-js/commit/10f3412fa27c2d8ca44ef7605250f4755d531be8))
* **errors:** updated error validation logic to handle charging availability error responses ([cf6ed5a](https://github.com/tomtom-international/go-sdk-js/commit/cf6ed5a646099a9dc76e0ba77a7fa2cd228a4ce7))
* expect any string to fix tests with changing data ([bcedd32](https://github.com/tomtom-international/go-sdk-js/commit/bcedd32074c8fcc7121c669938ca413a814d5ca4))
* fix import from core, fix connector type, improve test to min dublications ([51265ec](https://github.com/tomtom-international/go-sdk-js/commit/51265ecc8c5af9a9f46734180f1b95f90f00641a))
* for failing test because of changing data order ([b032098](https://github.com/tomtom-international/go-sdk-js/commit/b032098d6f3aa6391e8190990b83e688ca779e90))
* increase map load and test timeout to avoid failing tests ([a3db3d8](https://github.com/tomtom-international/go-sdk-js/commit/a3db3d86b84a89270cb2a047fbad4d3f23fe9db2))
* mock rtl plugin in tests ([f8a5736](https://github.com/tomtom-international/go-sdk-js/commit/f8a573680b82956df9a3b783bc74f674f6db29d9))
* replace set-out with env var ([c95f1d8](https://github.com/tomtom-international/go-sdk-js/commit/c95f1d889348e52c4d033e1a44193623a342e2fb))
* **services:** add correct score field to all relevant types ([a758514](https://github.com/tomtom-international/go-sdk-js/commit/a758514776820d11bba9e030fc22f753dfa1e023))
* **view:** change exp type to enum to be uniform with other services ([86cfcb6](https://github.com/tomtom-international/go-sdk-js/commit/86cfcb6b6bff7151dfd53b83e2c90c1d8e65edc6))

### [0.1.46](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.44...v0.1.46) (2022-10-19)


### Features

* **map:** initial support for sdk map modules based on maplibre ([b5e4ee0](https://github.com/tomtom-international/go-sdk-js/commit/b5e4ee00dab6afb9b54a6ea888f63f8539c9b6bf))


### Bug Fixes

* **deps:** fixing import statement ([57aca4f](https://github.com/tomtom-international/go-sdk-js/commit/57aca4fcf789e9fcb64393fad4be859eaf463747))

### [0.1.45](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.44...v0.1.45) (2022-10-19)


### Features

* **map:** initial support for sdk map modules based on maplibre ([b5e4ee0](https://github.com/tomtom-international/go-sdk-js/commit/b5e4ee00dab6afb9b54a6ea888f63f8539c9b6bf))


### Bug Fixes

* **deps:** fixing import statement ([57aca4f](https://github.com/tomtom-international/go-sdk-js/commit/57aca4fcf789e9fcb64393fad4be859eaf463747))

### [0.1.44](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.43...v0.1.44) (2022-10-17)


### Features

* add veracode pipeline ([03f0895](https://github.com/tomtom-international/go-sdk-js/commit/03f089513d61b967e89b026ab6dc620e9c1b8f9d))
* calc latest version ([30fac89](https://github.com/tomtom-international/go-sdk-js/commit/30fac8961fdef9d4d9ad47e6f8cbf1a0357d2113))
* run veracode scan after build workflow is successful ([a09cc3e](https://github.com/tomtom-international/go-sdk-js/commit/a09cc3e45dccd9abf8763b6ba1257ff1e05b14b9))


### Bug Fixes

* different name for PR ([9d57398](https://github.com/tomtom-international/go-sdk-js/commit/9d5739865a8af63f5ae641aa5681db40e7548339))

### [0.1.43](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.42...v0.1.43) (2022-10-11)


### Bug Fixes

* **versioning:** select correct latest version of alias ([b49a997](https://github.com/tomtom-international/go-sdk-js/commit/b49a997e8264ba7aab65997b11c5c4cd7a0a84c1))

### [0.1.42](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.41...v0.1.42) (2022-10-06)


### Features

* **map:** first iteration to export maplibre ([190ab52](https://github.com/tomtom-international/go-sdk-js/commit/190ab52c1f76c91854aa914021ace7f46ef11d88))

### [0.1.41](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.40...v0.1.41) (2022-10-06)


### Features

* customize obj for geometry search ([416c2e5](https://github.com/tomtom-international/go-sdk-js/commit/416c2e51d5f37b29476572aa032085ee530c39aa))
* export helper object per service ([1b21739](https://github.com/tomtom-international/go-sdk-js/commit/1b217393a8d4f9f150ac143bf14d8c76f4e0b2d6))

### [0.1.40](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.39...v0.1.40) (2022-10-05)


### Features

* **86-place-by-id:** Adding place by id service, work in progress ([187877e](https://github.com/tomtom-international/go-sdk-js/commit/187877e44be8d0e6fc2e784ffaf64e58e65f2dac))
* **86-place-by-id:** Adding place by id service, work in progress ([235288f](https://github.com/tomtom-international/go-sdk-js/commit/235288f396b12c8e26ee199a5f1099dde88e8ced))
* **map:** switching to maplibre ([63df229](https://github.com/tomtom-international/go-sdk-js/commit/63df229c5f031cfa059d23e24773aa84b450694b))
* **places:** building geometry data service and refactoring feature ids ([34e7e29](https://github.com/tomtom-international/go-sdk-js/commit/34e7e299e3f62cb27c714d4130061ad4cf401274))


### Bug Fixes

* **86-place-by-id:** code update after rebase ([38dd4a1](https://github.com/tomtom-international/go-sdk-js/commit/38dd4a1816f13604b786c8cfeaa49615e2fc70a4))
* **86-place-by-id:** duplication code removal ([3f7b856](https://github.com/tomtom-international/go-sdk-js/commit/3f7b856d960a482c5e1a92f58f1e2478c7d1f159))
* **86-place-by-id:** review comment ([743e20e](https://github.com/tomtom-international/go-sdk-js/commit/743e20ede79b3af5d8580e97a7aa77f61b0867bf))

### [0.1.39](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.38...v0.1.39) (2022-10-03)

### [0.1.38](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.37...v0.1.38) (2022-09-28)


### Features

* **services:** calculating bounding boxes for different service results ([38d3c91](https://github.com/tomtom-international/go-sdk-js/commit/38d3c913a6f1ad4777f8fb81b48821e1649ada21))


### Bug Fixes

* **deps:** updating package lock ([3377cdf](https://github.com/tomtom-international/go-sdk-js/commit/3377cdf5e0a492c49406ddfa44d6dc3bc856978b))
* expected test message correction ([d03dafe](https://github.com/tomtom-international/go-sdk-js/commit/d03dafee4111254e7fc43418294bf1956fb10f3a))

### [0.1.37](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.36...v0.1.37) (2022-09-22)


### Bug Fixes

* **validation:** Fix typo ([639cb4a](https://github.com/tomtom-international/go-sdk-js/commit/639cb4ae1579bf2ff3e68461f4b988900353a1bc))

### [0.1.36](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.35...v0.1.36) (2022-09-22)


### Bug Fixes

* **geometry-search:** misc type fixes and improvements ([f25ffa3](https://github.com/tomtom-international/go-sdk-js/commit/f25ffa3300cd029e36e72158381c1f05399c9175))

### [0.1.35](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.34...v0.1.35) (2022-09-22)


### Bug Fixes

* **geometry-search:** fixing type exports and namings ([0967214](https://github.com/tomtom-international/go-sdk-js/commit/09672148896031b11a78deb07fcf84cf2f73d88b))
* **geometry-search:** removing redundant type prop ([d39d682](https://github.com/tomtom-international/go-sdk-js/commit/d39d68279e4a1e42e20f88dc07cffc8c0748c2be))
* **geometry-search:** simplifying types ([4c39032](https://github.com/tomtom-international/go-sdk-js/commit/4c39032dab56ebade98335bf6262d313c1523ef8))

### [0.1.34](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.33...v0.1.34) (2022-09-22)


### Features

* **validation:** Add Validation error response type ([56784b8](https://github.com/tomtom-international/go-sdk-js/commit/56784b85885eda8e2f346b85ce59c65c84dcd449))
* **validation:** WIP - Add basic validation ([11753e1](https://github.com/tomtom-international/go-sdk-js/commit/11753e1f88a5e1d2ae080df9f65c56b97efc22c5))


### Bug Fixes

* [#373](https://github.com/tomtom-international/go-sdk-js/issues/373) bug fix ([8d36348](https://github.com/tomtom-international/go-sdk-js/commit/8d363486f8dbf1a77970ea285dd23452c31d8644))
* **geometry-search:** send missing parameters ([113d768](https://github.com/tomtom-international/go-sdk-js/commit/113d768bd66fc3c185d33fa4b0f87aaebc0014d7))
* **validation:** Fix typo ([46639ab](https://github.com/tomtom-international/go-sdk-js/commit/46639ab46acaba0836b7b9c656191ebc6cdb4f7d))
* **validation:** Renaming/add types ([6678dae](https://github.com/tomtom-international/go-sdk-js/commit/6678dae401aa2fa3b5f53e10896bf7dba83d75b3))

### [0.1.33](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.32...v0.1.33) (2022-09-13)


### Features

* **geometry-search:** implement geometry search api call ([f0eccdf](https://github.com/tomtom-international/go-sdk-js/commit/f0eccdfa0669b4e3a1c97d414fc376954c45eb2a))
* **map:** first draft version of map initialization based on current wip renderer and misc improvements ([7d7f955](https://github.com/tomtom-international/go-sdk-js/commit/7d7f95505c76f28097f18e11c7c9224332eb51b6))


### Bug Fixes

* **error-handler:** Add geocode support ([19b7373](https://github.com/tomtom-international/go-sdk-js/commit/19b73738e44a697aea3ceb63c3e2063f335c925a))
* **versioning:** regex should be resistant to backtracking ([086adbe](https://github.com/tomtom-international/go-sdk-js/commit/086adbe0644a3b339fa7a246f80ad3e3ea388bd8))

### [0.1.32](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.31...v0.1.32) (2022-09-08)

### [0.1.31](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.30...v0.1.31) (2022-09-08)


### Bug Fixes

* **cicd:** fixing github actions ([55848c4](https://github.com/tomtom-international/go-sdk-js/commit/55848c41e671d92a792acbf30a3982674c5ee3de))

### [0.1.30](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.29...v0.1.30) (2022-09-08)


### Bug Fixes

* **actions:** Install before switch version ([33783f6](https://github.com/tomtom-international/go-sdk-js/commit/33783f65ed9f56266a4fa77398abecc83efc551a))
* **error-handler:** Add routing error handler ([506db39](https://github.com/tomtom-international/go-sdk-js/commit/506db390cbfd4c87f8e2cb9339c0973ce5b5cd63))
* **tests:** making performance tests more stable ([f15afa9](https://github.com/tomtom-international/go-sdk-js/commit/f15afa9063034006930ce9e1af43991ca295a9e2))

### [0.1.29](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.28...v0.1.29) (2022-09-05)

### [0.1.28](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.27...v0.1.28) (2022-09-05)


### Features

* ignore internal functions ([3b6c605](https://github.com/tomtom-international/go-sdk-js/commit/3b6c60512257904e845f8871de8a8f258ff9b1cc))
* skip internal methods from definition files ([be0af50](https://github.com/tomtom-international/go-sdk-js/commit/be0af50461cc4e3e3c75a04a16935e3f8842ceff))


### Bug Fixes

* correct the types property to point to the dist decleration file ([2ac075f](https://github.com/tomtom-international/go-sdk-js/commit/2ac075ff6a48bcb1c625147f1eedf3464cf52079))
* **eslint:** downgrading eslint for now due to a bug found in their latest version ([c6ad928](https://github.com/tomtom-international/go-sdk-js/commit/c6ad92802fc544ae49c7445772a94046fc69b3b0))
* **npm:** exporting types for lodash and geojson ([3b80dc5](https://github.com/tomtom-international/go-sdk-js/commit/3b80dc5059de861b8bc0560f22e111aa23ea1025))
* remove src directory form the bundle ([ec1d735](https://github.com/tomtom-international/go-sdk-js/commit/ec1d7357601e099f9de8502afeff632180088a89))
* **sonar:** minor typo fix ([0439b84](https://github.com/tomtom-international/go-sdk-js/commit/0439b84368c6a92d8bf1cf9894c8d1e54390efa7))

### [0.1.27](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.26...v0.1.27) (2022-08-31)


### Features

* **error-handler:** Add first version of error handler ([c011740](https://github.com/tomtom-international/go-sdk-js/commit/c011740a1ed6a3ac391fa4e91bcc7328f9179a65))


### Bug Fixes

* **error-handler:** Removing Error.test ([6cdddb0](https://github.com/tomtom-international/go-sdk-js/commit/6cdddb0fb4026b7cf224b87251d47033f96e92ab))
* **error-handler:** Testing error object to contain correct status. ([3f72a67](https://github.com/tomtom-international/go-sdk-js/commit/3f72a67b03751d29d8dc85810a4b8ae2dd4a76ba))
* **error-handler:** Using the default message from API if any ([e19e222](https://github.com/tomtom-international/go-sdk-js/commit/e19e222d442503ad9202aaf0dd3d54ab75b99f3b))

### [0.1.26](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.25...v0.1.26) (2022-08-30)

### [0.1.25](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.24...v0.1.25) (2022-08-30)


### Features

* **routing:** adding support for vehicle parameters ([c5a49dd](https://github.com/tomtom-international/go-sdk-js/commit/c5a49dd111654e8535502e4e6c104635261003b1))


### Bug Fixes

* attempt to parameterize tests ([6687cda](https://github.com/tomtom-international/go-sdk-js/commit/6687cdab28d736cf0fb6f69fcc235ee0d58d5b93))
* ignore docs and sonar props file from node package ([0b8896b](https://github.com/tomtom-international/go-sdk-js/commit/0b8896bbb91b42df748cd747f30f59d699bfed5a))

### [0.1.24](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.23...v0.1.24) (2022-08-25)


### Features

* **routing:** adding support for vehicleheading api parameter and fixing considertraffic param bug ([e3e01c1](https://github.com/tomtom-international/go-sdk-js/commit/e3e01c1203a972d0fb52bd1fcf120c158619d7b9))

### [0.1.23](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.22...v0.1.23) (2022-08-22)

### [0.1.22](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.20...v0.1.22) (2022-08-22)


### Features

* geocoding service ([977ab9f](https://github.com/tomtom-international/go-sdk-js/commit/977ab9f1d1988e7048aa84e4c2cba3b7c2b03131))
* **routing:** building route planning service ([d0fb7a0](https://github.com/tomtom-international/go-sdk-js/commit/d0fb7a0757b63a2f29c1b4be3dbafd6c36c7eeab))
* **sonar:** add sonar metrics to the readme file ([2cb75a0](https://github.com/tomtom-international/go-sdk-js/commit/2cb75a03d57ddcdbc64bc9108e45ca02cd8d407f))


### Bug Fixes

* **eslint:** fix linter setup and errors ([459a5a6](https://github.com/tomtom-international/go-sdk-js/commit/459a5a659f71c0e0601372e2ec7b6c68d8e6c05e))
* **eslint:** ignore files should include the generated resources directories ([8473d5e](https://github.com/tomtom-international/go-sdk-js/commit/8473d5e0a13814054c2ba3cd9b4753523eecb4fd))
* fix tests to overcome unstable id results ([daaa543](https://github.com/tomtom-international/go-sdk-js/commit/daaa5436da94f3ca4d4e25cc9c17ebca08c2bf57))
* **i18n:** language is always optional ([da09003](https://github.com/tomtom-international/go-sdk-js/commit/da090039ae8f54da47e70321dbfd41db6d79b82e))
* **npm:** ability to calculate the old re-created package versions ([04ddaa5](https://github.com/tomtom-international/go-sdk-js/commit/04ddaa51c851264c8a211fee3dca241575a262f6))
* **services:** fixing rev geo types, improving tests, refactoring ([3f44a8f](https://github.com/tomtom-international/go-sdk-js/commit/3f44a8f208585b53c01bf69bb7c695f1ea1e9b12))
* **tests:** do not put the undefined key to url ([407eb9a](https://github.com/tomtom-international/go-sdk-js/commit/407eb9a992d0e3ae6615ce5b38ae56d322e61496))
* update tests ([e0fdf10](https://github.com/tomtom-international/go-sdk-js/commit/e0fdf10f170a3db49b212182a5902041c662b0c2))
* **workspaces:** ts build should use aliased local dependencies ([0bf2f4c](https://github.com/tomtom-international/go-sdk-js/commit/0bf2f4cf8739209af5b04502094da714b499ea66))

### [0.1.21](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.20...v0.1.21) (2022-08-22)


### Features

* geocoding service ([977ab9f](https://github.com/tomtom-international/go-sdk-js/commit/977ab9f1d1988e7048aa84e4c2cba3b7c2b03131))
* **routing:** building route planning service ([d0fb7a0](https://github.com/tomtom-international/go-sdk-js/commit/d0fb7a0757b63a2f29c1b4be3dbafd6c36c7eeab))
* **sonar:** add sonar metrics to the readme file ([2cb75a0](https://github.com/tomtom-international/go-sdk-js/commit/2cb75a03d57ddcdbc64bc9108e45ca02cd8d407f))


### Bug Fixes

* **eslint:** fix linter setup and errors ([459a5a6](https://github.com/tomtom-international/go-sdk-js/commit/459a5a659f71c0e0601372e2ec7b6c68d8e6c05e))
* **eslint:** ignore files should include the generated resources directories ([8473d5e](https://github.com/tomtom-international/go-sdk-js/commit/8473d5e0a13814054c2ba3cd9b4753523eecb4fd))
* fix tests to overcome unstable id results ([daaa543](https://github.com/tomtom-international/go-sdk-js/commit/daaa5436da94f3ca4d4e25cc9c17ebca08c2bf57))
* **i18n:** language is always optional ([da09003](https://github.com/tomtom-international/go-sdk-js/commit/da090039ae8f54da47e70321dbfd41db6d79b82e))
* **npm:** ability to calculate the old re-created package versions ([04ddaa5](https://github.com/tomtom-international/go-sdk-js/commit/04ddaa51c851264c8a211fee3dca241575a262f6))
* **services:** fixing rev geo types, improving tests, refactoring ([3f44a8f](https://github.com/tomtom-international/go-sdk-js/commit/3f44a8f208585b53c01bf69bb7c695f1ea1e9b12))
* **tests:** do not put the undefined key to url ([407eb9a](https://github.com/tomtom-international/go-sdk-js/commit/407eb9a992d0e3ae6615ce5b38ae56d322e61496))
* update tests ([e0fdf10](https://github.com/tomtom-international/go-sdk-js/commit/e0fdf10f170a3db49b212182a5902041c662b0c2))
* **workspaces:** ts build should use aliased local dependencies ([0bf2f4c](https://github.com/tomtom-international/go-sdk-js/commit/0bf2f4cf8739209af5b04502094da714b499ea66))

### [0.1.20](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.19...v0.1.20) (2022-07-27)


### Bug Fixes

* skip sonarqube workflow if github actor is dependabot ([5a0cab8](https://github.com/tomtom-international/go-sdk-js/commit/5a0cab8bfa5317ee770aa1caf058e46d7fa1f5ed))
* sonarqube ignore branch syntax ([a4e9764](https://github.com/tomtom-international/go-sdk-js/commit/a4e976445b585c1b093f3a4c0625cf2e998c9e79))

### [0.1.19](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.18...v0.1.19) (2022-07-12)


### Features

* add coverage report ([6c6e529](https://github.com/tomtom-international/go-sdk-js/commit/6c6e529ffc0013e31d8a77fc75d952d2422c64fc))
* **axios:** bringing axios back, switching to jest axios mock ([b884323](https://github.com/tomtom-international/go-sdk-js/commit/b88432337b63f370847cecf91c65604a344bc89b))
* configure sonarQube ([1d7cd6d](https://github.com/tomtom-international/go-sdk-js/commit/1d7cd6d72bb4dd9af0ab6dc35154e28608214cd7))
* ignore test files from coverage analysis ([b44ece2](https://github.com/tomtom-international/go-sdk-js/commit/b44ece223600fd8aa922b92edd6000cd3aaef08f))
* wildcard for coverage filepaths ([443b5c7](https://github.com/tomtom-international/go-sdk-js/commit/443b5c71428ac50f956f23c4e8688d9497e0857d))


### Bug Fixes

* **axios:** fix tests ([182efb8](https://github.com/tomtom-international/go-sdk-js/commit/182efb812e0643ea3e3ca0ba9e02f67a1cf32ee3))

### [0.1.18](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.17...v0.1.18) (2022-07-12)


### Features

* **axios:** bringing axios back, switching to jest axios mock ([fe17db2](https://github.com/tomtom-international/go-sdk-js/commit/fe17db2e4291fb4b7e288f2eef3a49abb27ebefb))
* **axios:** bringing axios back, switching to jest axios mock ([6ffd077](https://github.com/tomtom-international/go-sdk-js/commit/6ffd077d9354a47cce6ab14f4ca7e4d2adb6e472))

### [0.1.17](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.16...v0.1.17) (2022-07-11)


### Bug Fixes

* **npm:** expose sub packages and types correctly ([ddd37d1](https://github.com/tomtom-international/go-sdk-js/commit/ddd37d1d64876a4d2eab793d8dbb191aa72451b7))

### [0.1.16](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.15...v0.1.16) (2022-07-11)


### Features

* change color for release and delete workflows ([8b321fb](https://github.com/tomtom-international/go-sdk-js/commit/8b321fbc9f105f87e00f22ef0ff1e2207468cf8f))
* colored slack notification border ([9544046](https://github.com/tomtom-international/go-sdk-js/commit/954404686af99028b7ea14bb704019f8d2fd76ea))


### Bug Fixes

* notification text ([2bfc551](https://github.com/tomtom-international/go-sdk-js/commit/2bfc55172476f5a7708bde5a9b662a766eda7f06))

### [0.1.15](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.14...v0.1.15) (2022-06-30)


### Features

* **rev-geo:** refactoring to create a reusable service template ([3938832](https://github.com/tomtom-international/go-sdk-js/commit/3938832f61cd3056d81b38583539efe16bff582b))
* **rev-geo:** refactoring to create a reusable service template ([da1576e](https://github.com/tomtom-international/go-sdk-js/commit/da1576ec3d6c357547c4d5fd7d22d4aa2a6d0901))
* **rev-geo:** refactoring update request and response calls, adding tests ([f4d884f](https://github.com/tomtom-international/go-sdk-js/commit/f4d884fe37c3176be63503bd29e7666800cd41ca))
* **rev-geo:** refactoring update request and response calls, adding tests ([183010c](https://github.com/tomtom-international/go-sdk-js/commit/183010c50b620984198f250eda4df2e4478b81eb))

### [0.1.14](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.13...v0.1.14) (2022-06-30)


### Bug Fixes

* **bundle:** exclude core from bundles of siblings ([1610d93](https://github.com/tomtom-international/go-sdk-js/commit/1610d9399324386b5bb0e799989e1fa40c618942))
* **type:** configuration merge function should accept partial cfg ([7a7a666](https://github.com/tomtom-international/go-sdk-js/commit/7a7a6666f73735e0eecdf2ef61f33ec634a3a38d))
* **type:** remove redundant config initialization ([7967424](https://github.com/tomtom-international/go-sdk-js/commit/7967424ab7951c9a547ee4943ca8ca0b29093846))

### [0.1.13](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.12...v0.1.13) (2022-06-28)

### [0.1.12](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.11...v0.1.12) (2022-06-28)

### [0.1.11](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.10...v0.1.11) (2022-06-28)


### Bug Fixes

* **actions:** disable npm caching temporary ([e6fff9e](https://github.com/tomtom-international/go-sdk-js/commit/e6fff9e0516a52b43c02908aa174a6affd2b4102))
* **hotfix:** github actions should react on all branch names ([385d06a](https://github.com/tomtom-international/go-sdk-js/commit/385d06ae23c6c390a18ee9555b19eb90837a8189))
* **hotfix:** uncomment caching for npm ([9710bab](https://github.com/tomtom-international/go-sdk-js/commit/9710bab3a50e71e1aadf757e20bad78e89e4206d))
* **hotfix:** use a correct alias name source for the deletion action ([69898bd](https://github.com/tomtom-international/go-sdk-js/commit/69898bde49bd9ebc175ee8a043922dbf851db6a7))
* **version:** calculate the latest version for aliases correctly ([0ba6aeb](https://github.com/tomtom-international/go-sdk-js/commit/0ba6aebd0ec4bfd9067f893db56f6d708d00c95e))

### [0.1.10](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.9...v0.1.10) (2022-06-07)

### [0.1.9](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.8...v0.1.9) (2022-06-07)

### [0.1.8](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.7...v0.1.8) (2022-06-07)

### [0.1.7](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.6...v0.1.7) (2022-06-07)

### [0.1.6](https://github.com/tomtom-international/go-sdk-js/compare/v0.1.5...v0.1.6) (2022-06-07)

### 0.1.5 (2022-06-07)


### Features

* **actions:** ability to release and remove npm branch packages ([5818162](https://github.com/tomtom-international/go-sdk-js/commit/58181622b539e3a14aeb8436e03a8883d2cab9e0))
* **npm:** enable test coverage calculation in dev and release builds ([f9ccc87](https://github.com/tomtom-international/go-sdk-js/commit/f9ccc87e3ed5ea035709b782ef32abc719fd6c6e))
* **npm:** github actions should support workspaces build ([5e526b8](https://github.com/tomtom-international/go-sdk-js/commit/5e526b868c604d6e2480281351dad9ce5e8f7476))
* **npm:** implement dev and prod bundles for cjs and es ([31a0360](https://github.com/tomtom-international/go-sdk-js/commit/31a03600e789ef3c8dbacef5e15052140221c287))
* **rev-geo:** adding more rev geo params and tests ([c5d33fb](https://github.com/tomtom-international/go-sdk-js/commit/c5d33fbb4c3532bf6d360ab5d68d2fc13b6c0837))
* **rev-geo:** adding more rev geo params and tests ([094d88d](https://github.com/tomtom-international/go-sdk-js/commit/094d88da818f4852a596ef98306a8f0f52e82df3))
* **rev-geo:** adding more rev geo params and tests ([5150624](https://github.com/tomtom-international/go-sdk-js/commit/51506243783205be097ac09263233c92039c12b7))
* **rev-geo:** adding more rev geo params and tests ([f9192f8](https://github.com/tomtom-international/go-sdk-js/commit/f9192f816abb635822987d3c5e4968bd2ac63dda))
* **rev-geo:** adding more rev geo params and tests ([837e7a6](https://github.com/tomtom-international/go-sdk-js/commit/837e7a61f5997f42a6fd9c0f6cbde92a9f7c20cf))
* **rev-geo:** refactoring, adding more types and parsing logic ([6431f03](https://github.com/tomtom-international/go-sdk-js/commit/6431f036808e14f51f4e0198adf5a8c8cef4d534))
* **rev-geo:** refactoring, adding more types and parsing logic ([1d7dd80](https://github.com/tomtom-international/go-sdk-js/commit/1d7dd80caec8c731f46243b7b64bd3907ce7a44b))
* **rev-geo:** refactoring, adding more types and parsing logic ([c928ed4](https://github.com/tomtom-international/go-sdk-js/commit/c928ed43d8d1a817f2cff7c811300a611bbe207b))
* **rev-geo:** refactoring, adding more types and parsing logic ([d53053c](https://github.com/tomtom-international/go-sdk-js/commit/d53053c763b82dc717d0da232fe60cb7872b291d))
* **rev-geo:** refactoring, adding more types and parsing logic ([90bdc4c](https://github.com/tomtom-international/go-sdk-js/commit/90bdc4c6993ac0d033a0a413d1a0489a02ee5aa2))
* **rev-geo:** refactoring, adding more types and parsing logic ([83b8f22](https://github.com/tomtom-international/go-sdk-js/commit/83b8f22a8e3cabc83c248c6641132ebbbe3ede9b))
* **rev-geo:** refactoring, adding more types and parsing logic ([d647d1d](https://github.com/tomtom-international/go-sdk-js/commit/d647d1d4b319530354ea3b2ca16a819875e2dcf6))
* **revgeo:** new basic global config and rev geo implementation progress ([c68da98](https://github.com/tomtom-international/go-sdk-js/commit/c68da98f286f0e1b18f7ed5131e06be94606d5f7))
* **revgeo:** rev geo implementation progress ([3b4e80b](https://github.com/tomtom-international/go-sdk-js/commit/3b4e80b64b76551e60c893f3b69cdf6908e31a26))
* **revgeo:** rev geo implementation progress ([1468df6](https://github.com/tomtom-international/go-sdk-js/commit/1468df6606330c39ce9b84984b3575338c622572))


### Bug Fixes

* **hotfix:** ability to release tags ([530ca4e](https://github.com/tomtom-international/go-sdk-js/commit/530ca4e7afed79e57160460e1f07484631c49c39))
* **hotfix:** node installation fix for the delete action ([9c14a12](https://github.com/tomtom-international/go-sdk-js/commit/9c14a121897dd34d6e6d6f676427d411f61fc74a))
* **hotfix:** release scripts should use npx ([81a6232](https://github.com/tomtom-international/go-sdk-js/commit/81a6232992dc798aa41c1dd38386e2ad7a9e9f9b))
* **scaffolding:** fixing artifactory password on base64 ([c665bd1](https://github.com/tomtom-international/go-sdk-js/commit/c665bd101dd06f1bbcd178a5af3129ff13c8bdb6))
* **scaffolding:** fixing package name to match artifactory config ([cbee93b](https://github.com/tomtom-international/go-sdk-js/commit/cbee93bff6528e04f74adddaeb1f84f96494f87a))
* **tests:** trying to fix tests runners ([44d4df9](https://github.com/tomtom-international/go-sdk-js/commit/44d4df9728fb03263c505d53d0ac366bafaefd3f))
