    ...
    return (
        <Root
            ...
            className={`flex w-full gap-2 border-0 bg-transparent px-3 py-2 text-left ${
                clickable ? 'cursor-pointer transition-colors hover:bg-(--ui-surface-1)' : ''
            }`}
        ...
        >
            ...
            <div className="min-w-0 flex-auto">
                <div className="truncate text-[12px] font-semibold text-(--ui-text-high-em)">{title}</div>
                {description != null && (
                    <div className="mt-0.5 line-clamp-2 text-[12px] leading-snug text-(--ui-text-med-em)">
                        {description}
                    </div>
                )}
                <div className="mt-1 flex items-center gap-2 text-[11px] text-(--ui-text-low-em)">
                    {tag && (
                        <span
                    ...
                    {unread && (
                        <span aria-label="unread" className="h-1.5 w-1.5 rounded-full bg-(--ui-surface-brand-red)" />
                    )}
                </div>
                {note != null && <div className="mt-0.5 text-[10px] text-(--ui-text-low-em)">{note}</div>}
            </div>
            {trailing != null && (
                <span className="mt-0.5 flex h-6 w-4 shrink-0 items-center justify-center text-(--ui-text-low-em)">
                    {trailing}
                </span>
            )}
        </Root>
