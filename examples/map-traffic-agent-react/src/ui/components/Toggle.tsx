...
const TONE_CLASS: Record<ToggleTone, string> = {
    success: 'peer-checked:bg-(--sdk-color-success)',
    primary: 'peer-checked:bg-(--sdk-primary-color)',
};
...
export function Toggle({
    ...
    const baseLabel =
        labelClassName === null
            ? ''
            : (labelClassName ??
              'flex cursor-pointer select-none items-center gap-3 py-1 text-[13px] leading-[18px] text-(--sdk-text-high)');

    const trackClass = `relative inline-block shrink-0 rounded-full bg-(--sdk-border-high) transition-colors duration-300 ease-in-out after:absolute after:left-0.5 after:top-0.5 after:rounded-full after:bg-(--sdk-surface-0) after:shadow-[0_2px_4px_rgba(0,0,0,0.2)] after:transition-transform after:duration-300 after:ease-in-out after:content-[''] peer-disabled:cursor-not-allowed peer-disabled:opacity-50 ${SIZE_CLASS[size]} ${TONE_CLASS[tone]}`;
