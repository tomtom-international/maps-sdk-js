...
const mapLayersPlaygroundInit = async () => {
    ...
    document.querySelector('#toggleFlow')?.addEventListener('click', () => flow.setVisible(!flow.isVisible()));

    const pois = await POIsModule.get(map);
    document.querySelector('#togglePOIs')?.addEventListener('click', () => pois.setVisible(!pois.isVisible()));
...
window.addEventListener('load', mapLayersPlaygroundInit);
